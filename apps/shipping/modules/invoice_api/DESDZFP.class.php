...
/**
 * Class DESDZFP 爱信诺电子发票Api数据加解密类
 */
class DESDZFP
{
	private $key = 'LmMGStGtOpF4xNyvYt54EQ==';
	
	private $iv;
	
	public function __construct($key = null)
	{
		$key = base64_decode($key ?: $this->key);
		
		$this->key = substr($key, 0, 8);
		
		$this->iv = substr($key, 8);
	}
	...
	public function encrypt($data)
	{
		...
		$data = $this->addMD5Hash($data);
		
		return DES::encode($data, $this->key, $this->iv);
	}
	...
	public function decrypt($data)
	{
		...
		$data = DES::decode($data, $this->key, $this->iv);
		
		return substr($data, 16);
	}

	private function addMD5Hash($data)
	{
		return md5($data, true) . $data;
	}
}

/**
 * Class DES 支持mcrypt未开启也照样能正常加解密
 * @internal 该类包含自定义加解密方法，当php未开启mcrypt扩展时自动调用自定义加解密方法
 */
class DES
{
	public static function isMcryptExtEnable()
	{
		return extension_loaded('mcrypt');
	}
	
	public static function encode($str, $key, $iv = null, $mode = MCRYPT_MODE_CBC)
	{
		$size = self::isMcryptExtEnable() ? mcrypt_get_block_size(MCRYPT_DES, $mode) : 8;
		
		$str = self::pkcs5Pad($str, $size);
		
		if( self::isMcryptExtEnable() )
			$str = mcrypt_encrypt(MCRYPT_DES, $key, $str, $mode, $iv);  // MCRYPT_ENCRYPT
		else 
			$str = self::_des($key, $str, 1, 1, $iv);
		
		return base64_encode($str);
	}

	public static function decode($str, $key, $iv = null, $mode = MCRYPT_MODE_CBC)
	{
		$strBin = base64_decode($str);
		
		if( self::isMcryptExtEnable() )
			$str = mcrypt_decrypt(MCRYPT_DES, $key, $strBin, $mode, $iv);  // MCRYPT_DECRYPT	
		else
			$str = self::_des($key, $strBin, 0, 1, $iv);
	...
	private static function _des($key, $message, $encrypt, $mode = 1, $iv = null)
	{
		if( $iv=='' ) $iv = $key;
