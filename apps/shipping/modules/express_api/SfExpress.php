...
class SfExpress {
        ...
        //const  _CHECKHEADER = 'BSPdevelop';//客户卡号,校验码
        //const _URL = 'http://bsp-ois.sit.sf-express.com:9080/bsp-ois/sfexpressService';//快递类服务接口url
        //const _CHECKWORD = 'j8DzkIFgmlomPt0aLuwU';//密钥
         
        const  _CHECKHEADER = '7559909389';//客户卡号,校验码
        const _URL = 'http://bsp-oisp.sf-express.com/bsp-oisp/sfexpressService';//快递类服务接口url
        const _CHECKWORD = 'QPuA4B7we3c5R16ZjIO7faCYJTnPbnYZ';//密钥
        ...
        public static function searchOrder($order_id){
            $body ="<?xml version='1.0' encoding='UTF-8' ?><Request service='OrderSearchService' lang='zh-CN'><Head>".self::_CHECKHEADER."</Head><Body><OrderSearch orderid='".$order_id."'/></Body></Request>";       
            $newbody = $body.self::_CHECKWORD;        
            ...
            $res = self::post(self::_URL,$parambody);   
            $res=Util::xmlToArray($res); 
            
            
            if($res && is_array($res)){
                if(isset($res['Head']) && $res['Head']=='OK' && isset($res['Body']['OrderResponse']['@attributes'])){
                    return array('result'=>1,
                        'express_no'=>$res['Body']['OrderResponse']['@attributes']['mailno'],
                        'express_order_id'=>$res['Body']['OrderResponse']['@attributes']['orderid'],
                        'destcode'=>$res['Body']['OrderResponse']['@attributes']['destcode'],
                        'detail'=>$res['Body']['OrderResponse']['rls_info']['rls_detail']['@attributes']
                );                   
                }else{
                    if($res['Head']=='ERR' && $res['ERROR']=='找不到该订单'){
                         return array('result'=>0,'error'=>'找不到该订单');                        
                    }else
                        return array('result'=>0,'error'=>'接口返回异常');                          
                }
            }else{
        ...
        public static function makeOrder($order_id,$data){
            //$res=self::searchOrder($order_id);
            ...
            //        if(isset($res['result']) && $res['result']==0 && $res['error']=='找不到该订单'){
                        $body ="<?xml version='1.0' encoding='UTF-8' ?><Request service='OrderService' lang='zh-CN'><Head>".self::_CHECKHEADER."</Head><Body><Order orderid='".$order_id."' is_gen_bill_no='1' pay_method='1' ";
                        if($data['express_type']<>'' && in_array($data['express_type'], array('1','2')))
                            $body.="  express_type='".$data['express_type']."' ";
                        else
                            $body.="  express_type='1' ";

                        if(!empty(EXPRESS_SF_YUEJIEZHANGHAO))
                            $body.="  custid='".EXPRESS_SF_YUEJIEZHANGHAO."' ";
                        else
                            return array('result'=>0,'error'=>'寄方月结，月结账号必填');  
 
                        if($data['j_company']<>'') 
                            $body.="  j_company='".$data['j_company']."' ";
                        if($data['j_contact']<>'') 
                            $body.="  j_contact='".$data['j_contact']."' ";
                        if($data['j_tel']<>'') 
                            $body.="  j_tel='".$data['j_tel']."' ";                        
                        if($data['j_address']<>'') 
                            $body.="  j_address='".$data['j_address']."' ";
                        if($data['d_company']<>'') 
                            $body.="  d_company='".$data['d_company']."' ";
                        if($data['d_contact']<>'') 
                            $body.="  d_contact='".$data['d_contact']."' ";
                        if($data['d_tel']<>'') 
                            $body.="  d_tel='".$data['d_tel']."' ";
                        if($data['d_address']<>'') 
                            $body.="  d_address='".$data['d_address']."' ";
                        //if($data['remark']<>'') 
                        //    $body.="  remark='".$data['remark']."' ";
                        

                           
                        $body.="  routelabelForReturn='1' routelabelService='1' url_flag='1' is_unified_waybill_no='1'/>";

                        if($data['goods_name']<>'')
                            $body.="<Cargo name='".$data['goods_name']."'></Cargo>";
                        else
                            return array('result'=>0,'error'=>'寄件货品名称不能为空');  

                        $body.="</Order></Body></Request>";
                        file_put_contents('send.log', $body);
                        $newbody = $body.self::_CHECKWORD;        
                        $md5 =  md5($newbody,true);  
                        $verifyCode = base64_encode($md5);           
                        $fields = array('xml'=>$body,'verifyCode'=>$verifyCode);
                        ...
                        $res = self::post(self::_URL,$parambody); 
                        file_put_contents('receive.log', $res);
                        $res=Util::xmlToArray($res);
                        //echo "<pre>"; print_r($res);
                        if(isset($res['Head']) && $res['Head']=='OK' && isset($res['Body']['OrderResponse']['@attributes']))
                            return array('result'=>1,
                        ...
                        elseif($res['Head']=='ERR')
                            return array('result'=>0,'error'=>isset($res['ERROR']['@attributes']['code']) ? $res['ERROR']['@attributes']['code'] : $res['ERROR']);
                        else
                            return array('result'=>0,'error'=>'接口异常');
        ...
        protected function citycode($province,$city){
          $expressname = array(  
