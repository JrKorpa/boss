...
class ZtExpress {
        ...
        const _URL = 'http://japi.zto.cn/gateway.do'; //'http://58.40.16.125:9001/gateway.do';//快递类服务接口url
        ...
        public static function makeOrder($order_id,$param){             
                ...
                $data['sender']['name']= $param['j_contact'];        
                $data['sender']['mobile']= $param['j_tel'];
                $data['sender']['phone']=  $param['j_tel'];        
                $data['sender']['city']= self::_J_CITY;
                $data['sender']['address']= $param['j_address'];
                $data['receiver']['name']= $param['d_contact'];
                $data['receiver']['mobile']= $param['d_tel'];
                $data['receiver']['phone']= $param['d_tel'];
                $data['receiver']['city']= !empty($param['city']) ? $param['city'] : mb_substr($param['d_address'],0,6);
                $data['receiver']['address']= $param['d_address'];
                ...
                if(is_array($datoubi) && $datoubi['status']==1 && is_array($datoubi['result']) && !empty($datoubi['result']['mark'])){
                ...
                //echo $data_digest;                
                $body=array('company_id' => self::_COMPANY_ID, 'msg_type' => 'PARTNERINSERT_SUBMITAGENT','data' => $data,'data_digest' => $data_digest);
                $res = self::post(self::_URL,$body);                  
                ...
                elseif(!empty($res['message']))
                        return array('result'=>0,'error'=>json_encode($res));
                else     
        ...
        public static function getDaTouBi($order_id,$province,$city,$district,$address){
            //echo $address;   
                ...
                $key=self::_KEY;
                $data_digest = base64_encode(md5($data.$key, TRUE));
                $body=array('company_id' => self::_COMPANY_ID, 'msg_type' => 'TRACEABLEMARK_GETMARK','data' => $data,'data_digest' => $data_digest);
                $res = self::post(self::_URL,$body);                  
                $res=json_decode($res,true);
                //print_r($res);                  
                return $res;     
        ...
        public static function getMoneys(){
                ...
                $data_digest = base64_encode(md5($data.$key, TRUE));
                //echo $data_digest;                
                $body=array('company_id' => self::_COMPANY_ID, 'msg_type' => 'PARTNERINSERT_AVAILABLECOUNTER','data' => $data,'data_digest' => $data_digest);
                $res = self::post(self::_URL,$body);                  
                $res=json_decode($res,true);   
