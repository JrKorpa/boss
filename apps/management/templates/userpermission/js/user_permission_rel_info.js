var user_permission_rel_info_user_id = '<%$user_id%>';
var user_permission_rel_info_parent_id = '<%$parent_id%>';

function user_permission_rel_selectall(o){
	...
	$('#user_permission_rel_info1>div>div').each(function(){
...
function user_permission_rel_save(o){
        var user_id = $("#user_permission_user_search_form input[name='user_id']").val();
        if (!parseInt(user_id))
        {
                $('.modal-scrollable').trigger('click');
	...
	var ids=[];
	$('#user_permission_rel_info1 .checkbuttonOk .checktext').each(function(){
		ids.push($(this).attr('id'))
	});
        App.blockUI({target: $('#user_permission_rel_list'), iconOnly: true});
	$.post('index.php?mod=management&con=UserPermission&act=saveRel',{user_id:user_permission_rel_info_user_id,parent_id:user_permission_rel_info_parent_id,ids:ids},function(data){
		if (data.success==1)
		{
...
$import(function(){
	$('#user_permission_rel_info1').disableSelection();
	$("#user_permission_rel_info1>div>div").click(
		function(event) {
