var user_button_list_user_id_20141229 = '<%$user_id%>';
var UserPermissionButtonListObj = function(){
	var handleTreeList = function(user_id){
		...
		function onClick(e,treeId, treeNode) {
			...
			if (treeNode.tid)
			{
				...
				$.post('index.php?mod=management&con=UserPermission&act=buttonList',{permission_id:treeNode.tid,user_id:user_id,rid:treeNode.rid},function(data){
					$('#user_permission_button_info').html(data);
                                        App.unblockUI($('#user_permission_button_list'));
				});
			}
			else
			{
				$('#user_permission_button_info').html('没有按钮');
			}
		}
		...
		$.ajax({
			   ...
			   success: function(data){
				   var i1=i2=true;
					...
					if (zNodes.length)
					{
						$("#user_permission_button_tree").addClass('ztree');
						$.fn.zTree.init($("#user_permission_button_tree"), setting, zNodes);
                                                App.unblockUI($('#user_permission_button_list'));
					}
					else
					{
						$("#user_permission_button_tree").html('没有菜单');
                                                App.unblockUI($('#user_permission_button_list'));
					}
				}
		});
	...
	return {
		init:function(){
			handleTreeList(user_button_list_user_id_20141229);
		}
	}
}();
...
$import(["public/js/jquery-ztree/css/zTreeStyle.css","public/js/jquery-ztree/js/jquery.ztree.core-3.5.js"],function(){
	UserPermissionButtonListObj.init();
});