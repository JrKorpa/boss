function user_warehouse_copy_check_all_channel(){
        var ids = [];
	$('#user_warehouse_copy_info select[name="house_id[]"] option').each(function(){
		if ($(this).val())
		{
			ids.push($(this).val());
		}
	});
	$('#user_warehouse_copy_info select[name="house_id[]"]').select2('val',ids).change();    
}
$import("public/js/select2/select2.min.js",function(){
	var info_form_id = 'user_warehouse_copy_info';//form表单id
			...
			$('#'+info_form_id+' select[name="user_id"]').select2({
				...
				if (_t)
				{
					var id = $('#'+info_form_id+' input[name="id"]').val();
                                        var uid = $('#'+info_form_id+' input[name="uid"]').val();
					$.post('index.php?mod=management&con=UserWarehouse&act=getHouses',{user_id:_t,id:id,uid:uid},function(data){
						$('#'+info_form_id+' select[name="house_id[]"]').append(data);
					});					
				}
				else
				{
					$('#'+info_form_id+' select[name="house_id[]"]').val('').change();
				}
