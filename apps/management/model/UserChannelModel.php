...
//					echo $sql;
    ...
    public function getChannels_class ($user_id,$id,$channel_class)
    {
        $sql = "SELECT c.id,c.channel_name FROM `".$this->table()."` AS u INNER JOIN `sales_channels` AS c ON u.channel_id=c.id AND c.is_deleted=0 WHERE u.`user_id`='{$user_id}' AND u.id<>".$id." AND `c`.`channel_class` = ".$channel_class;
        return $this->db()->getAll($sql);
    }
