 ...
 *  -------------------------------------------------
 */
class UpdateOrderAdvisorController extends CommonController {
    /**
     * 	index
     */
    public function index($params) {
        ...
        $this->render('update_order_advisor_info.html', array('bar' => Auth::getBar()));
    }
    /**
     * 	update，更新信息
     */
    public function update_name($params) {
        ...
        if($order_sn){
            ...
            foreach($tmp as $k => $val){
                ...
                if($order_info_ct['create_user'] == $kehuname){
                    $res['msg'] = "名字已是“".$order_info_ct['create_user']."”，无需修改";
                    Util::jsonExit($res);
                }
                $order_sn_list .= "'$val',";
				$order_id_list .= "'{$order_info_ct['id']}',";
                $order_sn_arr[$k] = $val;
				$create_user_arr[$k]=$order_info_ct['create_user'];
        ...
        $where['create_user'] = $kehuname;
        $ret = $orderModel->updateOrderAdvisorBySn($where);
		foreach ($order_info_ct as $val) {
            $orderinfos[$val['id']] = $val['create_user'];
        }
        foreach ($order_sn_arr as $k=>$val) {
            ...
            $logInfo = array(
                ...
                'remark'=>'顾问姓名 '.$create_user_arr[$k].' 修改为 '.$kehuname,
            );
            //写入订单日志
