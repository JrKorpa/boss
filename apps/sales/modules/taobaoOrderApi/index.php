...
class taobaoOrderApi
{
	...
	function taobaoOrderApi()
	{
		if(1 || empty( $this->top_session))
		{
			...
			*/
		    ...
		    //$this -> top_session   = '61018107437cd80b9610e50332fa67ed91bc334e9c8b3ba92449256';
		    $this -> top_session = '6101f218a94eb4f2895069ed6fce818f98e300209a1209d92449256';
		}
	}
	...
	*/
	function make_order_info($taobao_order_id)
	{
		...
		$isyushou =0 ;
		foreach($tb_items as $tmp_g)
		{
			$gtitle = $tmp_g->title;
			//var_dump($tmp_g);
			if(strpos($gtitle,'YS')!==false || strpos($gtitle,'预售')!==false){
				$isyushou = 1;
				break;	
			}
		}
	...
	function outer_order_pay($order_info, $taobao_order_sn, $price=false)
	{
        ...
        //等待卖家发货状态
		$tb_items = array();
        if (is_array($taobao_order_info->trade->orders->order)) {
		    $tb_items = $taobao_order_info->trade->orders->order;
		} else {
		    $tb_items[] =  $taobao_order_info->trade->orders->order;
		}
		$isyushou =0 ;
        ...
        if(trim($taobao_order_info->trade->status) != "WAIT_SELLER_SEND_GOODS") {
			if($isyushou<1){ 
				return 2;
			}
		}
