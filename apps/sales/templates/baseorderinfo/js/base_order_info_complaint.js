$import(function(){
	var info_form_id = 'base_order_info_complaint';//form表单id
...
//			});	
		...
		var handleForm = function(){
			var url = info_form_base_url+(info_id ? 'ins_complaint' : 'ins_complaint');
			var options1 = {
				url: url,
				error:function ()
				{
						...
						util.xalert(
							...
							function(){
								...
								else
								{
									...
									{//刷新首页
										app_order_feedback_search_page(util.getItem("orl"));
									}
								}
							}
						);
...
//					}
		...
		var initData = function(){
			$('#'+info_form_id+' :reset').on('click',function(){
				...
				//单选按钮组重置
				$("#"+info_form_id+" input[name='cl_feedback_id'][value='"+cl_feedback_id+"']").attr('checked','checked');
				var test = $("#"+info_form_id+" input[name='cl_feedback_id']:not(.toggle, .star, .make-switch)");
				if (test.size() > 0) {
					test.each(function () {
						if ($(this).parents(".checker").size() == 0) {
							$(this).show();
