...
$import(["public/js/select2/select2.min.js","public/js/bootstrap-datepicker/js/bootstrap-datepicker.js","public/js/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js","public/js/fancyapps-fancyBox/jquery.fancybox.css","public/js/fancyapps-fancyBox/jquery.fancybox.js"],function(){
	...
	util.setItem('listDIV1','app_order_goods_info_detail');
	util.setItem('orl2','index.php?mod=sales&con=AppOrderInvoice&act=search&order_id='+order_id+'&_id='+getID().split('-').pop());//设定刷新的初始url
	util.setItem('listDIV2','app_order_invoice_info_detail');
    
	util.setItem('orl4','index.php?mod=sales&con=BaseOrderInfo&act=showLogs&id='+order_id);//设定刷新的初始url
	util.setItem('listDIV4','base_action_info_search_list');
...
$("#gifts input[type='checkbox']").click(function(){
	var check =  $(this).attr('checked');
	var val= $(this).val();
	if(check=="checked"){
		$('#gifts input[name="gift_num['+val+']"]').css('display','inline');
	}else{
		$('#gifts input[name="gift_num['+val+']"]').css('display','none');
		$('#'+val).remove(); 
	}
});