$import(function(){
	...
	var obj = function(){
		...
		var handleForm = function(){
			...
			var url = 'index.php?mod=sales&con=BaseOrderInfo&act=saveOrderGoods&goods_id='+goods_id+"&id="+id;
			var options1 = {
				url: url,
				data:formdata,
				error:function ()
			...
			$('#app_order_detail_goods_form').validate({
				...
				submitHandler: function (form) {
					$("#app_order_detail_goods_form").ajaxSubmit(options1);
				}
			});
			//回车提交
			$('#app_order_detail_goods_form input').keypress(function (e) {
				if (e.which == 13) {
					$('#app_order_detail_goods_form').validate().form()
				}
			});
		};
