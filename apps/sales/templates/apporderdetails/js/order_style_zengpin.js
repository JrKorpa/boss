$import(['public/js/jquery.sinaEmotion.css',"public/js/select2/select2.min.js"],function(){
	...
	var obj = function(){
		var initElements = function(){
            $('#app_order_detail_zengpin_goods_form select').select2({
                placeholder: "请选择",
                allowClear: true,
		...
		var handleForm = function(){
			...
			$('#app_order_detail_zengpin_goods_form').validate({
				...
				submitHandler: function (form) {
					$("#app_order_detail_zengpin_goods_form").ajaxSubmit(options1);
				}
			});
			//回车提交
			$('#app_order_detail_zengpin_goods_form input').keypress(function (e) {
				if (e.which == 13) {
					$('#app_order_detail_zengpin_goods_form').validate().form()
				}
			});
		};
		var initData = function(){
            $('#app_order_detail_zengpin_goods_form :reset').click(function(){
                $('#add_zengpin_goods').trigger('click');
            });
		};
		return {
			init:function(){
