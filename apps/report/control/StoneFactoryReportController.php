 ...
 * 工厂石包汇总
 *  -------------------------------------------------
 *   @file		: StoneFactoryReportController.php
 *   @link		:  www.kela.cn
 ...
 *  -------------------------------------------------
 */
class StoneFactoryReportController extends CommonController
{
    ...
    public function index($param){
        ...
        $processors_id = _Request::getInt('id');
        $this->render('stone_factory_report_search_form.html',array('bar'=>Auth::getBar(),
                'pro_list'=>$pro_list,
    ...
    public function search($param){
        ...
        $data = $model->pageListStoneFactory($where,$page,10,false);
        $pageData = $data;
        $pageData['filter'] = $args;
        $pageData['jsFuncs'] = 'stone_factory_report_search_page';
        $this->render('stone_factory_report_search_list.html', array(
    ...
    public function export($param){
        ...
        header("Content-Disposition: attachment;filename=工厂石包汇总报表.csv");
        header('Cache-Control: max-age=0');
    
        $model = new ShibaoModel(45);
    
        $titleArr=array('工厂名称','供应商','石包号','总数量(粒)','总重量(ct)','总金额');
        ...
        $pageCount=1;
        $recordCount = 0;
        $list_sql=$model->getListStoneFactorySql($where);
        //无限循环拉取数据
