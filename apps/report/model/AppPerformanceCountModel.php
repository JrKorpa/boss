...
AND  `b`.bill_type =  'D' ";
	...
	**/
	...
	{
		$tt = 0;
		$sql  = "SELECT `a`.`id`,`a`.`order_sn`,b.goods_sn,`b`.`id` as detailsid,`b`.`favorable_price`,
		`b`.`favorable_status`,`b`.`goods_price`,`b`.`goods_type`,`b`.`goods_count`,a.`is_zp`,
		`b`.`is_return`,`b`.`cart`,`b`.`xiangqian`, `a`.`pay_date`,`a`.`mobile`,g.product_type1,`p`.`product_type_name`,
		if(LENGTH(b.cert) >0,b.cert,g.zhengshuleibie) as cert,`b`.`zhengshuhao` ";
		$sql .=" FROM `".$this->table()."` as a left join `app_order_details` as b on ";
		$sql .= " `a`.`id`=`b`.`order_id` ";
		$sql .=" left join `warehouse_shipping`.`warehouse_goods` as g on `b`.`goods_id`=`g`.`goods_id` left join front.base_style_info bi on bi.style_sn = b.goods_sn 
	...
	**/
	...
	{
		$sql  = "SELECT `a`.`id`,`b`.`favorable_price`,`b`.`favorable_status`,`b`.`goods_price`,`b`.`goods_type`,`b`.`goods_count` ,`b`.`is_return`";
