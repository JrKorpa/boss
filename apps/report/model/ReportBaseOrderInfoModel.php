...
/**
 *  -------------------------------------------------
 *   @file		: ReportBaseOrderInfoModel.php
 *   @link		:  www.kela.cn
 *   @update	:
 *  -------------------------------------------------
 */
class ReportBaseOrderInfoModel extends Model
{
	...
	function pageList ($where,$page,$pageSize=10,$useCache=true)
	{
		...
		if(isset($where['diff_day']) && $where['diff_day']){
			//*天发货订单
			$sql="SELECT temp_b.create_time send_goods_time,c.create_time pay_time,`b`.`order_amount`,`b`.`money_paid`,`b`.`money_unpaid`,`b`.`goods_return_price`,`b`.`real_return_price`,`b`.`shipping_fee`,`b`.`goods_amount`,`b`.`coupon_price`,`b`.`favorable_price`,`b`.`card_fee`,`b`.`pack_fee`,`b`.`pay_fee`,`b`.`insure_fee`,`a`.*
				FROM app_order.`base_order_info` a
		...
		)  temp_b ON temp_b.`order_id`=a.id
		JOIN app_order_account b ON b.order_id=a.id
		LEFT JOIN (
		SELECT temp1.create_time ,temp1.`order_id`,temp1.action_id FROM app_order.`app_order_action` temp1
		INNER JOIN (SELECT MIN(action_id) action_id FROM app_order.`app_order_action` WHERE pay_status IN (2,3) GROUP BY order_id ) temp2
		ON temp1.`action_id`=temp2.action_id
		)
		c ON c.`order_id`=a.id
			where 1=1 ";
			if($where['time_type']!='add'){
				$sql.=" AND a.check_time!='' AND  c.create_time!=''";
			}
			$diff_day=$where['diff_day'];
			$diff_next_day=$diff_day-1>0?$diff_day-1:0;
			if($diff_day<=7){
				$sql.=" and  DATEDIFF(temp_b.create_time,c.create_time)<{$diff_day} and  DATEDIFF(temp_b.create_time,c.create_time)>={$diff_next_day} and temp_b.create_time and c.create_time ";
			}
			elseif($diff_day<=20){
				$sql.=" and  DATEDIFF(temp_b.create_time,c.create_time)>={$diff_next_day} and  DATEDIFF(temp_b.create_time,c.create_time)<20  and temp_b.create_time and c.create_time  ";
			}
			else{
				$sql.=" and  DATEDIFF(temp_b.create_time,c.create_time)>=20";
			}
		}
		elseif($where['time_type']=='add'){//日期为下单时间
			$sql  = "SELECT `b`.`order_amount`,`b`.`money_paid`,`b`.`money_unpaid`,`b`.`goods_return_price`,`b`.`real_return_price`,`b`.`shipping_fee`,`b`.`goods_amount`,`b`.`coupon_price`,`b`.`favorable_price`,`b`.`card_fee`,`b`.`pack_fee`,`b`.`pay_fee`,`b`.`insure_fee`,`a`.* FROM `".$this->table()."` as a
						left JOIN  cuteframe.`sales_channels` s ON s.id=a.department_id
					join `app_order_account` as b on  `a`.`id`=`b`.`order_id`";
			$sql .= " where 1=1 ";
		}
		elseif($where['time_type']=='send'){//日期为发货时间
			$sql="SELECT `b`.`order_amount`,`b`.`money_paid`,`b`.`money_unpaid`,`b`.`goods_return_price`,`b`.`real_return_price`,`b`.`shipping_fee`,`b`.`goods_amount`,`b`.`coupon_price`,`b`.`favorable_price`,`b`.`card_fee`,`b`.`pack_fee`,`b`.`pay_fee`,`b`.`insure_fee`,`a`.* 
			FROM base_order_info a
			left JOIN  cuteframe.`sales_channels` s ON s.id=a.department_id
			LEFT JOIN ( SELECT temp1.create_time ,temp1.`order_id`,temp1.action_id FROM app_order.`app_order_action` temp1 INNER JOIN (SELECT MIN(action_id) action_id FROM app_order.`app_order_action` WHERE shipping_status=2 GROUP BY order_id ) temp2 ON temp1.`action_id`=temp2.action_id ) temp_b 
 ON temp_b.`order_id`=a.id 
 LEFT JOIN ( SELECT temp1.create_time ,temp1.`order_id`,temp1.action_id FROM app_order.`app_order_action` temp1 INNER JOIN (SELECT MIN(action_id) action_id FROM app_order.`app_order_action` WHERE pay_status IN (2,3) GROUP BY order_id ) temp2 ON temp1.`action_id`=temp2.action_id ) c 
 ON c.`order_id`=a.id 
 JOIN app_order_account b ON b.order_id=a.id
			WHERE 1=1 AND a.check_time!='' AND  c.create_time!='' AND temp_b.create_time!='' ";
		}
		...
		if(!empty($where['start_time'])){
        		...
        		$sql.=" AND `temp_b`.`create_time` >= '".$where['start_time']." 00:00:00'";
        }
        if(!empty($where['end_time'])){
        	if($where['time_type']=='add'){
        		$sql.=" AND `a`.`create_time` <= '".$where['end_time']." 23:59:59'";
        	}else
        		$sql.=" AND `temp_b`.`create_time` <= '".$where['end_time']." 23:59:59'";
        }
		...
		if((isset($where['diff_day']) && $where['diff_day']) || $where['time_type']!='add'){
			//重新计算分页;
			$countSql = preg_replace('/^(SELECT.+?\bFROM\b)/i', 'SELECT COUNT(*) count FROM', $sql, 1);
			$data['recordCount'] = count($this->db()->getAll($countSql,$params=array(),$useCache=false));
       ...
       if(empty($order_id)){
		...
		if(empty($val['is_peishi'])){
			$val['is_peishi'] =0;
				...
				if (empty($vals[0])) $vals[0] = 2;
				if (empty($vals[1])) $vals[1] = 0;
...
// 			file_put_contents('makeemptyordergoods',serialize($e));
	...
	public function getGoodsPrice($order_id){
         $sql = "SELECT *  FROM `app_order`.`app_order_details` WHERE `order_id`={$order_id} AND is_return = 1";
...
if(isset($order_data['user_id']) && !empty($order_data['user_id'])){
    ...
    public function addOderDetail($goods){
        ...
        $field = "`order_id`,`goods_id`,`goods_sn`,`ext_goods_sn`,`goods_name`,`goods_price`,`goods_count`,`create_time`,`create_user`,`modify_time`,`details_status`,`is_stock_goods`,`details_remark`,`cut`,`cart`,`clarity`,`color`,`zhengshuhao`,`caizhi`,`jinse`,`jinzhong`,`zhiquan`,`kezi`,`face_work`,`xiangqian`,`send_good_status`,`buchan_status`,`goods_type`,`is_zp`,`is_finance`,`favorable_price`,`favorable_status`";
        $value = " ".$order_id." ,'".$goods['goods_id']."' ,'".$goods['goods_sn']."' ,'".$goods['ext_goods_sn']."' ,'".$goods['goods_name']."' ,".$goods['goods_price']." ,".$goods['goods_count']." , '".$goods['create_time']."' , '".$goods['create_user']."', '".$goods['modify_time']."', ".$goods['details_status'].", ".$goods['is_stock_goods'].", '".$goods['details_remark']."','".$goods['cut']."','".$goods['cart']."','".$goods['clarity']."','".$goods['color']."','".$goods['zhengshuhao']."','".$goods['caizhi']."','".$goods['jinse']."','".$goods['jinzhong']."','".$goods['zhiquan']."','".$goods['kezi']."','".$goods['face_work']."','".$goods['xiangqian']."',1,1,'".$goods['goods_type']."','".$goods['is_zp']."','".$goods['is_finance']."','".$goods['favorable_price']."','".$goods['favorable_status']."'";
    ...
    public function changgestu($order_id){
        ...
        if($res['order_amount']<=$res['money_paid']){
            $dsql = "SELECT COUNT(*) FROM `app_order_details` WHERE `order_id`=$order_id AND `buchan_status`<>9 AND `buchan_status`<>11 AND is_stock_goods=0";
    ...
    public function originalDetaile($order_sn, $orderi,$goods_list){
        ...
        foreach($goods_list as $key=>$goods){
            $goods_value= " (".$res['id']." ,'".$goods['goods_id']."' ,'".$goods['goods_sn']."' ,'".$goods['goods_name']."' ,".$goods['goods_price']." ,".$goods['goods_count']." , '".$goods['create_time']."' , '".$goods['create_user']."', '".$goods['modify_time']."', ".$goods['details_status'].", ".$goods['is_stock_goods'].", '".$goods['details_remark']."','".$goods['cart']."','".$goods['clarity']."','".$goods['color']."','".$goods['zhengshuhao']."','".$goods['caizhi']."','".$goods['jinse']."','".$goods['jinzhong']."','".$goods['zhiquan']."','".$goods['kezi']."','".$goods['face_work']."','".$goods['xiangqian']."',1,1,'"."',".$goods['favorable_price'].",".$goods['favorable_status'].",".$goods['is_zp'].",".$goods['is_finance'].")";
            $sql_goods ="INSERT INTO `app_order_details` (" . $goods_field . ") VALUES $goods_value";
            $pdo->query($sql_goods);
            //插入关联表
            $id = $pdo->lastInsertId();
    ...
    public function selectzpinfo($goods_sn,$id) {
