...
class ProductFactoryOprauserModel extends Model
{
	...
	*/
	...
	/** 绑定跟单人到工厂上 **/
	public function BingGendanMan($pro_id , $user_id){
		...
		if($exists !== false){
			$sql = "UPDATE `product_factory_oprauser` SET `opra_user_id` = {$user_id} , `opra_uname`= '{$bing_user_name}' WHERE `prc_id` = {$pro_id}";
		}else{
			//写入主表
			$sql = "INSERT INTO `product_factory_oprauser` (`prc_id`,`opra_user_id`,`opra_uname`,`add_user`,`add_time`) VALUES ({$pro_id} , {$user_id} , '{$bing_user_name}' , '{$user}' , '{$time}')";
		}
