...
//            "balance_day" => "结算日期",
    ...
    function pageList($where, $page, $pageSize = 10, $useCache = true) {
        if (isset($where['opra_uname']) && $where['opra_uname'] != "") {
            $sql = "SELECT `app_processor_info`.*,`product_factory_oprauser`.`opra_uname` FROM `" . $this->table() . "`, `product_factory_oprauser` WHERE `app_processor_info`.`id` = `product_factory_oprauser`.`prc_id` ";
	...
	{
		$sql = "SELECT `id`,`code`,`name` FROM ".$this->table()." WHERE 1";
		return $this->db()->getAll($sql);
	}
    /**
     * getList 获取通过供应商
