//导出csv
function upoffshelf_export_cxv_details(obj){
	...
	param['cangku'] = tObj[0].getAttribute("cangku");
	param['dotime_string'] = tObj[0].getAttribute("dotime_string");
...
$import(['public/js/select2/select2.min.js','public/js/bootstrap-datepicker/js/bootstrap-datepicker.js','public/js/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js'], function(){
	var url='index.php?mod=report&con=UpOffShelf&act=warehouse_detail_list&dt='+dt+'&warehouse_type='+warehouse_type+'&time_start='+time_start+'&time_end='+time_end+'&warehouse_string='+warehouse_string+'&company_id='+company_id;
	...
	var obj = function(){
		var initElements = function(){
			$('#warehouse_upoffshelf_search_second_form select').select2({
				placeholder: "请选择",
				allowClear: true,
			}).change(function(e){
				$(this).valid();
			...
			//单选美化
			var test = $("#warehouse_upoffshelf_search_second_form input[type='radio']:not(.toggle, .star, .make-switch)");
			if (test.size() > 0) {
				test.each(function () {
		...
		var initData = function(){
			...
			$('#warehouse_upoffshelf_search_second_form :reset').on('click',function(){
				//下拉置空
				$('#warehouse_upoffshelf_search_second_form select[name="type"]').select2('val','').change();//single
				$('#warehouse_upoffshelf_search_second_form select[name="status"]').select2('val','').change();//single
			});
		}
		return {
			init:function(){
