function show_wait_diamond_list_second(obj){
...
//分页
function wait_diamond_search_page_second(url){
	util.page(url);
}
//匿名回调
$import(["public/js/bootstrap-datepicker/js/bootstrap-datepicker.js",
    ...
    var xx_url = 'index.php?mod=report&con=WaitDiamondReport&act=search_second';
	...
	util.setItem('formID','wait_diamond_search_form_second');//设定搜索表单id
	util.setItem('listDIV','wait_diamond_search_list_second');//设定列表数据容器id
	//匿名函数+闭包
	var obj = function(){

		var initElements = function(){
                     //下拉列表美化
                            $('#wait_diamond_search_form_second select').select2({
                                placeholder: "全部",
                                allowClear: true,
                            }).change(function(e) {
                                $(this).valid();
                            ...
                            $('#wait_diamond_search_form_second :reset').on('click',function(){
				$('#wait_diamond_search_form_second select').select2("val","");
			    });
				$('#wait_diamond_search_form_second input[name=checkAll]').click(function(){
					var status=$(this).attr('checked');
					var arr=new Array();
					if(status=='checked'||status==true)
					{

						$('#wait_diamond_search_form_second select[name="buchan_fac_opra[]"] option').each(function(key,v){
							arr[key]=$(this).val();
						})
						$('#wait_diamond_search_form_second select[name="buchan_fac_opra[]"]').select2("val",arr);
					}
					$('#wait_diamond_search_form_second select[name="buchan_fac_opra[]"]').select2("val",arr);
