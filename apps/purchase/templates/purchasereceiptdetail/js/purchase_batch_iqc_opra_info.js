$import('public/js/select2/select2.min.js',function(){
	var obj = function(){
		...
		var handleForm = function(){
			...
			var url = 'index.php?mod=purchase&con=PurchaseReceiptDetail&act=batch_insertIqc';
			var opt = {
				url: url,
		...
		var initData = function(){
			$('#purchase_iqc_opra :reset').on('click',function(){
			//单选按钮组重置
				$("#purchase_iqc_opra input[name='opra_code'][value='"+opra_code+"']").attr('checked','checked');
				var test = $("#purchase_iqc_opra input[name='opra_code']:not(.toggle, .star, .make-switch)");
				if (test.size() > 0) {
					test.each(function () {
						if ($(this).parents(".checker").size() == 0) {
