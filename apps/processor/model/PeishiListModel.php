...
class PeishiListModel extends Model
{
	...
	function pageList ($where,$page,$pageSize=10,$useCache=true,$orderby="orderby1")
	{
		...
		//不要用*,修改为具体字段
		$sql = "SELECT 	p.`id`,	p.`rec_id`,	p.`order_sn`,`sc`.`channel_name`,p.`peishi_status`,p.stone_position,
        ...
        if(isset($where['is_quick_diy']) && $where['is_quick_diy']!=""){
            $str .= " AND pi.is_quick_diy = ".$where['is_quick_diy'];
        }
        ...
        if(!empty($where['department_id'])){
            $str .= " and oi.department_id = '{$where['department_id']}'";
        }
