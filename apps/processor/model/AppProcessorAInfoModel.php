 ...
 *  -------------------------------------------------
 */
class AppProcessorAInfoModel extends Model {
...
//            "balance_day" => "结算日期",
    ...
    function pageList($where, $page, $pageSize = 10, $useCache = true) {
        if (isset($where['opra_uname']) && $where['opra_uname'] != "") {
            $sql = "SELECT `app_processor_info`.*,`product_factory_oprauser`.`opra_uname` FROM `" . $this->table() . "`, `product_factory_oprauser` WHERE `app_processor_info`.`id` = `product_factory_oprauser`.`prc_id` and is_A_company = 'Y'";
	...
	function getProList()
	{
		$sql = "SELECT `id`,`code`,`name` FROM ".$this->table()." WHERE `status` = 1";
