...
class ApiStyleModel
{
   ...
   public function GetStyleInfoBySn($style_sn){
    ...
    //根据sn取出款式内容
   public function GetStyleGoods($goods_sn){
        $keys=array('goods_sn');
        $vals=array($goods_sn);
        $ret=ApiModel::style_api($keys,$vals,'GetStyleGoods');
        return $ret;
    }

    //根据sn取出款式内容
   public function GetStyleInfoAll($style_sn){
        $keys=array('style_sn');
        $vals=array($style_sn);
        $ret=ApiModel::style_api($keys,$vals,'GetStyleInfoAll');
        return $ret;
    }
