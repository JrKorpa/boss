$import("public/js/select2/select2.min.js",function(){
	var info_form_id = 'app_lz_discount_grant_info';//form表单id
	var info_form_base_url = 'index.php?mod=diamond&con=AppLzDiscountGrant&act=';//基本提交路径
	var info_id= '';
...
//			}
            ...
            $('#'+info_form_id+' :reset').on('click',function(){
                $('#'+info_form_id+' select[name="type"]').select2("val","");
            })
...
function get_diamond_dicount_mima(){
	var zhekou = $("#app_lz_discount_grant_info input[name='zhekou']").val();
	var type = $("#app_lz_discount_grant_info select[name='type']").val();
	var user_name = $("#app_lz_discount_grant_info input[name='user_name']").val();
	if(user_name == ""){
		util.xalert("请输入销售顾问");
		return false;
	}
    ...
    $.post('index.php?mod=diamond&con=AppLzDiscountGrant&act=createCode',{'zhekou':zhekou,'type':type,'user_name':user_name},function(data){
		if(data.success ==1){
			$("#app_lz_discount_grant_info input[name='mima']").val(data.error);
		}else{
			alert(data.error);
		}
