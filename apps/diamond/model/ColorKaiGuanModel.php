 ...
 *  -------------------------------------------------
 */
class ColorKaiGuanModel extends Model {
...
*/				"Suashish"=>array(
    ...
    function __construct($id = NULL, $strConn = "") {
        $this->_objName = 'diamond_color_from_ad';
     ...
     */
    function getAllList() {
        $sql = "SELECT `id`, `from_ad`, `enabled` FROM `" . $this->table() . "` ORDER BY `id` DESC";
    ...
    function UpdateKaiQi($kai,$guan){
        if(!empty($kai)){
            $sql="UPDATE `".$this->table()."` SET enabled=1 WHERE from_ad in (".implode(',',$kai).")";
// 			echo $sql;exit;
            $this->db()->query($sql);
        }
        if(!empty($guan)){
            $sql="UPDATE `".$this->table()."` SET enabled=0 WHERE from_ad in (".implode(',',$guan).")";
//             echo $sql;exit;
            $this->db()->query($sql);
        }
        return true;
