 ...
 *  -------------------------------------------------
 */
class DiamondJiajialvModel extends Model {

    public static $optertion_list = array(1 => '添加', 2 => '修改', 3 => '停用', 4 => '启用');

    function __construct($id = NULL, $strConn = "") {
        $this->_objName = 'diamond_jiajialv';
        $this->pk = 'id';
        $this->_prefix = '';
    ...
    function getAllList($select = "*", $where = array()) {
        ...
        $sql = " SELECT `id`,`good_type`,`from_ad`,`cert`,`carat_min`,`carat_max`,`jiajialv`,`status` FROM `" . $this->table() . "`" . $sql;
    ...
    public function array_difficult($newdo, $olddo) {
        ...
        $diamondView = new DiamondInfoView($newdo['id']); //不能直接new View，需要传一个主键id或传一个model对象
