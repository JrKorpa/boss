 ...
 *  -------------------------------------------------
 */
class DiamondGetJxcModel extends Model {
     ...
     * 获取裸钻数据
     */
    public function get_warehouse_by_houseId($filter)
    {
        if(isset($filter['id'])){
            $keys[]='warehouse_id';
            $vals[]=$filter['id'];
        ...
        if(isset($filter['page_size'])){
            $keys[]='page_size';
            $vals[]=$filter['page_size'];
        }
        $ret=ApiModel::warehouse_api($keys,$vals,'getDiamondDataByHouseId');
    ...
    public function insertByGroup($filter)
    {
        ...
        $insert_sql = "replace into `" . $this->table() . "` (`".implode('`,`',$field)."`) values ".trim($values,",");
