...
/**
 *  -------------------------------------------------
 *   @file		: OfficeDiscountConfigController.php
 *   @link		:  tsyd.kela.cn
 ...
 *  -------------------------------------------------
 */
class OfficeDiscountConfigController extends CommonController
{
	...
	{
		$OfficeDiscountConfig = new OfficeDiscountConfigView(new OfficeDiscountConfigModel(99)); 
		$ChannelsModel = new SalesChannelsModel(1);
		$channelinfos = $ChannelsModel->getAllChannelInfo();
		$discounttype = $OfficeDiscountConfig->get_discount_type();
		$this->render('office_discount_config_index.html',array('bar'=>Auth::getBar(),'discounttype'=>$discounttype,'channelinfos'=>$channelinfos));
	}
	...
	*/
	public function search(){
		...
		$args = array(
			...
			'act'	=> __FUNCTION__,
            'channel_name' => _Request::getString("channel_name"),
        ...
        $ChannelsModel = new SalesChannelsModel(1);
       	...
       	if(!empty($args['channel_name'])){
            $channel_id = $ChannelsModel->getChannelIdByChannelName($args['channel_name']);
            $where = array(
        ...
        $discount_model =new OfficeDiscountConfigModel(99);
		...
		$pageData['jsFuncs'] = 'Office_discount_config_search_page';
		$this->render('office_discount_config_search_list.html',array(
	...
	public function add ()
	{
		...
		//所有渠道
		$channelinfos = $ChannelsModel->getAllChannelInfo();
		$result['content'] = $this->fetch('office_discount_config_info.html',array(
			'view'=>new OfficeDiscountConfigView(new OfficeDiscountConfigModel(99)),
	...
	{
		$id = intval($params["id"]);
        $newmodel = new OfficeDiscountConfigModel(99);
		...
		$result = array('success' => 0,'error' => '');
		$result['content'] = $this->fetch('office_discount_config_info.html',array(
			'view'=>new OfficeDiscountConfigView(new OfficeDiscountConfigModel($id,99)),
			'tab_id'=>$tab_id,
			'channelinfos'=>$channelinfos,
	...
	public function update ($params)
	{
		...
		$id = _Post::getInt('id');
         ...
         for($i=1;$i<6;$i++){
            ...
            if($discount_data){
                $id = $discount_data['id'];
                $newmodel = new OfficeDiscountConfigModel($id,99);
	...
	public function delete ($params)
	{
		$result = array('success' => 0,'error' => '');
		$id = intval($params['id']);
		$model = new OfficeDiscountConfigModel($id,99);
