...
$import(["public/js/bootstrap-datepicker/js/bootstrap-datepicker.js","public/js/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js", "public/js/select2/select2.min.js"],function(){	util.setItem('orl','index.php?mod=warehouse&con=MaterialBill&act=search');//设定刷新的初始url
    util.setItem('orl','index.php?mod=warehouse&con=MaterialBill&act=search');//设定刷新的初始url
...
function printBill(obj){
	...
	$.post("index.php?mod=warehouse&con=MaterialBill&act=printBill",{id:id},function(res){
		if(res.error){
			alert(res.error);
		}else{
			var id = tObj[0].getAttribute("data-id").split('_').pop();
