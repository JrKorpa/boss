//表格初始化
var from_table_data_p = function(id , tdata , ttitle , ttype){
	var $container = $(id);
 ...
 // 批量获取添加输入值
    function getBatchInfo(idArry,x,n,obj){  //id 查询条件， table的x行数
		...
		var wholesale_user = $("#wholesale_user").val();
		...
		if(is_invoice==""){
			util.xalert("请选择是否开发票");
			return false;
		}
		var jsonData = {'g_ids':idArry,'bill_id':bill_id,'from_company':from_company,'wholesale_user':wholesale_user,'is_invoice':is_invoice};
	   <%else%>	
	    var jsonData = {'g_ids':idArry,'bill_id':bill_id,'from_company':from_company,'wholesale_user':wholesale_user}; 
	   <%/if%>	
        $.get("index.php?mod=warehouse&con=WarehouseBillInfoP&act=getGoodsInfos",jsonData, function(data){
