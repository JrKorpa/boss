...
function downCsv(obj){
	...
	var down_url = 'index.php?mod=warehouse&con=WarehousePandianPlan&act=downCsv&id='+id;
...
$import(['public/js/select2/select2.min.js','public/js/bootstrap-datepicker/js/bootstrap-datepicker.js','public/js/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js'], function(){
	util.setItem('orl','index.php?mod=warehouse&con=WarehousePandianPlan&act=search');//设定刷新的初始url
	util.setItem('formID','warehouse_pandian_plan_search_form');//设定搜索表单id
	util.setItem('listDIV','warehouse_pandian_plan_search_list');//设定列表数据容器id

	//匿名函数+闭包
	var obj = function(){
		var initElements = function(){
			$('#warehouse_pandian_plan_search_form select').select2({
				placeholder: "请选择",
				allowClear: true,
			}).change(function(e){
				$(this).valid();
			...
			//单选美化
			var test = $("#warehouse_pandian_plan_search_form input[type='radio']:not(.toggle, .star, .make-switch)");
			if (test.size() > 0) {
				test.each(function () {
		...
		var initData = function(){
			...
			$('#warehouse_pandian_plan_search_form :reset').on('click',function(){
				//下拉置空
				$('#warehouse_pandian_plan_search_form select[name="type"]').select2('val','').change();//single
				$('#warehouse_pandian_plan_search_form select[name="status"]').select2('val','').change();//single
			});
		}
		return {
			init:function(){
...
function printBill(obj){
	...
	var id=$(tObj).attr('data-title');

	$.post("index.php?mod=warehouse&con=WarehousePandianPlan&act=printBill",{id:id},
	function(res){
		...
		if(res.error){
			...
			var id = tObj[0].getAttribute("data-id").split('_').pop();
			var url = "index.php?mod=warehouse&con=WarehousePandianPlan&act=printBill";			
