// 表格初始化
var from_table_data_bill_b = function (id,tdata,title,tcolumns) {
  ...
  afterChange: function(change, source) { //改变内容之后执行的方法
        ...
        beforeRemoveRow: function(index,amount) {
		           ...
		           var amount;
		           var goods_num=parseInt(goods_num_old)-parseInt(amount);
				   $("#goods_num").val(goods_num);
      			   //计算删除后的价格总计
      			   var mingyijia=$("#mingyijia").val(); 
      			   var price=this.getDataAtRow(index);               	
                   var price_all = parseInt(mingyijia)- parseInt(price[2]);
    ...
    function getBatchInfo(idArry,x,n,obj){  //id 查询条件， table的x行数
        ...
        $.get("index.php?mod=warehouse&con=WarehouseBillInfoB&act=getGoodsInfos", {'g_ids':idArry,'bill_id':bill_id,'from_company':from_company,'in_warehouse_type':in_warehouse_type}, function(data){
