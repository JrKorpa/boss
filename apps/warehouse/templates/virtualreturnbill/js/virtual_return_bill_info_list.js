$import(['public/js/select2/select2.min.js'],function(){
	var info_form_id = 'virtual_return_bill_info_list';//form表单id
	var info_form_base_url = 'index.php?mod=warehouse&con=VirtualReturnBill&act=';//基本提交路径
...
//				minimumInputLength: 2
            ...
            $('#'+info_form_id+' select[name="from_company_id"]').select2({
                placeholder: "请选择",
                allowClear: true,
            }).change(function (e){
                $(this).valid();
                ...
                if (_t) {
                    $.post('index.php?mod=warehouse&con=VirtualReturnBill&act=getTowarehouseId', {'id': _t}, function (data) {
                        $('#'+info_form_id+' select[name="from_warehouse_id"]').attr('disabled', false).empty().append('<option value=""></option>').append(data);
                        $('#'+info_form_id+' select[name="from_warehouse_id"]').change();
                    });
                }else{
                    $('#'+info_form_id+' select[name="from_warehouse_id"]').attr('disabled', 'disabled').empty().append('<option value=""></option>').select2('val','');
                }
            });

            $('#'+info_form_id+' select[name="out_company_id"]').select2({
                placeholder: "请选择",
                allowClear: true,
            }).change(function (e){
                $(this).valid();
                ...
                if (_t) {
                    $.post('index.php?mod=warehouse&con=VirtualReturnBill&act=getTowarehouseId', {'id': _t}, function (data) {
                        $('#'+info_form_id+' select[name="out_warehouse_id"]').attr('disabled', false).empty().append('<option value=""></option>').append(data);
                        $('#'+info_form_id+' select[name="out_warehouse_id"]').change();
                    });
                }else{
                    $('#'+info_form_id+' select[name="out_warehouse_id"]').attr('disabled', 'disabled').empty().append('<option value=""></option>').select2('val','');
                }
            });
            ...
            $('#'+info_form_id+' select[name="place_company_id"]').select2({
                ...
                b=_t.split("|");
                if (_t) {
                    $.post('index.php?mod=warehouse&con=VirtualReturnBill&act=getTowarehouseId', {'id': b[0]}, function (data) {
		...
		var handleForm = function(){
			var url = info_form_base_url+(info_id ? 'update_pl' : 'update_pl');
			var options1 = {
				url: url,
				error:function ()
				{
