...
//打印条码
	...
	var baseMemberInfoInfogObj = function(){
    	...
    	$('#warehouse_bill_info_l_selectsalepolicy select[name="policy_type"]').select2({
				placeholder: "请选择",
				allowClear: true,
				...
				$(this).valid();
                var url ="index.php?mod=warehouse&con=WarehouseBillInfoL&act=getPolicytypeInfo"
                var policy_type=$(this).val();
                $.post(url,{policy_type:policy_type,goods_id:goods_id,policy_goods:policy_goods}, function (data) {
                    if(data.success==1){
                        $('#warehouse_bill_info_l_selectsalepolicy input[name=sta_value]').val(data.msg1);
                       $('#warehouse_bill_info_l_selectsalepolicy input[name=jiajialv]').val(data.msg);
                       }
                       else{
		...
		var handleForm = function(){
			//表单验证和提交
			var url = bill_id ? 'index.php?mod=warehouse&con=WarehouseBill&act=printcode' : 'index.php?mod=Warehouse&con=WarehouseBill&act=printcode';
			
        

			$('#warehouse_bill_info_l_selectsalepolicy').validate({
                ...
                submitHandler: function(form) {
                   var policy_type= $('#warehouse_bill_info_l_selectsalepolicy select[name="policy_type"]').val();
                
                    location.href = "index.php?mod=warehouse&con=WarehouseBill&act=printcode&bill_id="+bill_id+"&policy_type="+policy_type;
                }
			});
			//回车提交
			$('#warehouse_bill_info_l_selectsalepolicy input').keypress(function (e) {
				if (e.which == 13) {
					if ($('#warehouse_bill_info_l_selectsalepolicy').validate().form()) {
						$('#warehouse_bill_info_l_selectsalepolicy').submit();
					}
					else
					{
						return false;
