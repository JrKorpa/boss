...
function get_shop_order_shipping(url){
	$.get(url, {sjs:Math.random()}, function(res){
		if(res){ 
		  $('#distribute_way_detail').html(res);
		}else{
		   $('#distribute_way_detail').html('');
		 }
	});
}
...
$import("public/js/select2/select2.min.js",function(){
	...
	util.setItem('shop_orl','index.php?mod=warehouse&con=WaitDistribution&act=getGoodsListByOrder&order_sn='+sn);//设定刷新的初始url
	util.setItem('listDIV','wait_order_goods_info_detail');//设定列表数据容器id
	util.setItem('shop_actl_url','index.php?mod=warehouse&con=WaitDistribution&act=getOrderActionlist&order_sn='+sn);//设定刷新的初始url
	util.setItem('shopship', 'index.php?mod=warehouse&con=WaitDistribution&act=getShopShipping&order_sn='+sn); //设定发货信息url
	//匿名函数+闭包
	var obj = function(){
		...
		var initData = function(){
			wait_order_goods_info_detail(util.getItem("shop_orl"));
		...
		var form_table = function(){
			...
			$('#wait_destribution_form_btn').click(function(){
        		...
        		if ((typeof express_id) != 'undefined' && ( express_id == '' || $.trim(express_id) == '')) {
        			bootbox.alert('请选择快递公司');
        			return false;
        		}
        		if ((typeof express_id) != 'undefined' && express_id !='10' && (typeof freight_no) != 'undefined' && ( freight_no == '' || $.trim(freight_no) == '')) {
    				bootbox.alert('请输入快递单号');
    				return false;
        		}
        		
        		if ((typeof express_id) != 'undefined' && express_id !='10' && (typeof freight_no) != 'undefined' ) {
        			var old_freight_no = $('#ship_freight_shoporder_form input[name="__freight_no"]').val();
        			if (old_freight_no != freight_no) {
        				bootbox.alert('请先点击 保存快递方式');
        				return false;
        			}
        		}
						...
						if(confirm('订单金额低于总成本，是否继续？')){
							$.post(url, data, function(res){
								if(res.success ==1 ){
									xiaozhang_submits = 0;
									$('.modal-scrollable').trigger('click');//关闭遮罩
									$('#sudu').html('<span>'+res.order_sn+'  发货成功......</span><br/>'+res.error);
									$('#tab_bill_m_1 .portlet-body').html(''); //清空
									$('#wait_order_goods_info_detail').html(''); //清空
									$('#wait_distribution_log_list').html(''); //清空
									$('#distribute_way_detail').html(''); //清空
		...
		var handleForm2 = function(){
			$('#xiaozhang_search button').click(function(){
				...
				if(order_sn == '') {
					...
					util.setItem('shop_orl','index.php?mod=warehouse&con=WaitDistribution&act=getGoodsListByOrder&order_sn='+order_sn);//设定刷新的初始url
					util.setItem('shop_actl_url','index.php?mod=warehouse&con=WaitDistribution&act=getOrderActionlist&order_sn='+order_sn);//设定刷新的初始url
					util.setItem('shopship', 'index.php?mod=warehouse&con=WaitDistribution&act=getShopShipping&order_sn='+order_sn); //设定发货信息url
					...
					$.get(url1 , '', function(res){
						if(res.success == 0){
						   ...
						   $goods_eles = $('#wait_distribution_goods_lists input[name="jxc_goods_id[]"]');
						   if ($goods_eles.length > 0) {
							   $goods_eles[0].focus();
						   }
						}
					});
