...
var info_form_base_url = 'index.php?mod=warehouse&con=VipPickDetails&act=';//基本提交路径
...
function loadXianhuo(tabIndex,data){
	var box = $("#"+info_form_id+" .js_list").eq(tabIndex);	
	if(data == null){
		//重置表单数据
...
$import(["public/js/select2/select2.min.js"],function(){
	var obj = function(){
		var initElements = function(){
		   ...
		   $("#"+info_form_id+" .js_goods_id").die("blur");
		   $("#"+info_form_id+" .js_goods_id").live("blur",function(event){		
				var goods_id = $.trim($(this).val());
				var tabIndex = $("#"+info_form_id+" .js_goods_id").index($(this));
				if(goods_id!=''){
				    $.ajax({
						type:"POST",
			...
			$("#"+info_form_id+" .js_style_sn").die("blur");
			$("#"+info_form_id+" .js_style_sn").bind("blur",function(event){	
	       ...
	       $("#"+info_form_id+" .js_add_goods_btn").click(function(){
			   var tabIndex = $("#"+info_form_id+" .js_add_goods_btn").index($(this));
			   var box = $(this).parent().parent().parent().parent();
			   var count = box.find(".js_list").length;
			   var goodsHtml = '<div class="row js_list">';
			   goodsHtml+='<div class="table_dep">'+(tabIndex+1)+'-'+(count+1)+'</div>';
			   goodsHtml+='<input type="hidden" name="goods[parent][]" value="'+(tabIndex+1)+'" />';
			   goodsHtml += $("#goodsHiddenBox").text();
			   goodsHtml +='</div>';
			   ...
			   box.find(".table_dep").each(function(i){
					$(this).html((tabIndex+1)+'-'+(i+2));	  							
			   });
		   });
		   $("#"+info_form_id+" .js_remove_goods_btn").die("click");
		   $("#"+info_form_id+" .js_remove_goods_btn").live("click",function(){	
			   if(!confirm("确定删除？")){
					return false;
			   }
			   var box = $(this).parent().parent().parent();
			   var parent = box.find("input[name='goods[parent][]']").val();
			   var boxParent = box.parent();			   
			   box.remove();			   
			   boxParent.find(".table_dep").each(function(i){
					$(this).html((parent)+'-'+(i+2));	  							
			   });
		...
		var handleForm = function(){
			var url = "index.php?mod=warehouse&con=VipPickDetails&act=pickGoodsSave";
			var options1 = {
				url: url,
				error:function ()
				{
