$import(function(){
	var info_form_id = 'warehouse_bill_info_y_jiajialv_info';//form表单id
	var info_form_base_url = 'index.php?mod=warehouse&con=WarehouseBillInfoYJiaJiaLv&act=';//基本提交路径
...
$("select[name='style_type_id']").change(function(){
        $.ajax({
            type: "POST",
            cache: false,
            data:{style_type_id:$(this).val()},
            url: "index.php?mod=warehouse&con=WarehouseBillInfoYJiaJiaLv&act=getJiaJiaLv",
            dataType: "json",
            success: function(res)
            {
                $("input[name='active_jiajialv']").val(res.content);
            },
...
$("input[value='active_now']").on("focus", function(){
	var dd = new Date(); 
	$("input[name='active_date']").val(dd.getFullYear().toString() + "-" +  ((dd.getMonth()+1)<10?"0"+(dd.getMonth()+1):(dd.getMonth()+1)).toString() + "-" + (dd.getDate()<10?"0"+dd.getDate():dd.getDate()).toString());
});
