...
$import(["public/js/bootstrap-datepicker/js/bootstrap-datepicker.js", "public/js/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js","public/js/select2/select2.min.js"],function(){
	util.setItem('orl','index.php?mod=warehouse&con=WaitDistributionTsyd&act=showList');//设定刷新的初始url
	util.setItem('formID','wait_distributiontsyd_search_form');//设定搜索表单id
	util.setItem('listDIV','wait_distributiontsyd_search_list');//设定列表数据容器id

	//匿名函数+闭包
	var obj = function(){
		$('#wait_distributiontsyd_search_form select').select2({
				placeholder: "请选择",
				allowClear: true
			});
		var initElements = function(){
			...
			$('#wait_distributiontsyd_search_form :reset').on('click',function(){
				$('#wait_distributiontsyd_search_form select').select2("val","");
			});
			$('#wait_distributiontsyd_search_form select[name="channel_class"]').select2({
				placeholder: "请选择",
				allowClear: true,
			}).change(function (e){
  				$(this).valid();
				...
				if (_t) {
					$.post('index.php?mod=warehouse&con=WaitDistribution&act=getChannelIdByClass', {'channel_class': _t}, function (data) {
						$('#wait_distributiontsyd_search_form select[name="sales_channels_id"]').attr('disabled', false).empty().append('<option value=""></option>').append(data);
						$('#wait_distributiontsyd_search_form select[name="sales_channels_id"]').change();
					});
				}else{
					$('#wait_distributiontsyd_search_form select[name="sales_channels_id"]').attr('disabled', 'disabled').empty().append('<option value=""></option>').select2('val','');
				}
			});
