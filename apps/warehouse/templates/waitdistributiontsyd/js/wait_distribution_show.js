...
function tab_next(obj){
	var input_obj = $('#wait_distribution_goods_lists input[name="jxc_goods_id[]"]');
	...
	if(parseInt(next) < parseInt(all)){
		input_obj.blur();
		$('#wait_distribution_goods_lists input[name="jxc_goods_id[]"]').eq(next).focus();
		/*
		var dds = now+'dds';
		var next = (parseInt(now)+1)+'dds';
		$('#'+dds).blur();
		$('#'+next).focus();
		*/
	}else{
...
$import("public/js/select2/select2.min.js",function(){
	...
	util.setItem('orl','index.php?mod=warehouse&con=WaitDistributionTsyd&act=getGoodsListByOrder&order_sn='+sn);//设定刷新的初始url
	util.setItem('listDIV','wait_order_goods_info_detail');//设定列表数据容器id
	util.setItem('actl_url','index.php?mod=warehouse&con=WaitDistributionTsyd&act=getOrderActionlist&order_sn='+sn);//设定刷新的初始url
	...
	var obj = function(){
		...
		var form_table = function(){
			$('#wait_destribution_form_btn').click(function(){
				...
				var url = 'index.php?mod=warehouse&con=WaitDistributionTsyd&act=xiaozhang';
		...
		var handleForm2 = function(){
			$('#xiaozhang_search button').click(function(){
				...
				if(order_sn == '') {
					...
					util.setItem('oarl','index.php?mod=warehouse&con=WaitDistributionTsyd&act=getGoodsListByOrder&order_sn='+order_sn);//设定刷新的初始url
					util.setItem('anactl_url','index.php?mod=warehouse&con=WaitDistributionTsyd&act=getOrderActionlist&order_sn='+order_sn);//设定刷新的初始url
					var url1 = "index.php?mod=warehouse&con=WaitDistributionTsyd&act=detail&ss=1&order_sn="+order_sn;
			...
			$('#xiaozhang_search input[name="order_sn"]').keypress(function (e) {
				if (e.which == 13)
				{
					...
					else
					{
						$('body').modalmanager('loading');//进度条和遮罩
						var order_sn = $('#xiaozhang_search input[name="order_sn"]').val();
						util.setItem('url_kp','index.php?mod=warehouse&con=WaitDistributionTsyd&act=getGoodsListByOrder&order_sn='+order_sn);//设定刷新的初始url
						util.setItem('otactl_url','index.php?mod=warehouse&con=WaitDistributionTsyd&act=getOrderActionlist&order_sn='+order_sn);//设定刷新的初始url
						var url1 = "index.php?mod=warehouse&con=WaitDistributionTsyd&act=detail&ss=1&order_sn="+order_sn;
						$.get(url1 , '', function(res){
							if(res.success == 0){
								$('body').modalmanager('removeLoading');//关闭进度条
