// 表格初始化
var from_table_data_ha_edit = function (id, tdata, ttitle, ttype , bill_id) {
// var from_table_data_bill_m = function (id,title,tcolumns) {
  var $container = $(id);
  //表格基本内容初始化
        ...
        afterChange: function (change, source) {//改变内容之后执行的方法
          if (source != "loadData" && change[0][1] == 0) {//跳过第一次/且第一列执行
            if(repeat(change[0][0],change[0][3])!= -1){
              util.xalert("录入信息重复",function(){
                this.alter("remove_row", change[0][0],1);//参数设定，remove_row方法名，change[0][0]删除的行数，1，删除1行
              });
              //$container.prev("p").addClass('text-danger').text("录入信息重复");
            }else if($container.find("td").hasClass("htInvalid") == true){
              util.xalert("录入信息错误！",function(){
                this.alter("remove_row", change[0][0], 1);
              });
            }else if($container.find("td").hasClass("htInvalid") == true){
              //$container.prev("p").addClass('text-danger').text("录入信息错误！");
              util.xalert("录入信息错误！",function(){
                this.alter("remove_row", change[0][0], 1);
              });
            }else{
				      var from_company_id = $("#to_company_id").val();
              obtain_info(change[0][3],change[0][0] ,this, bill_id,from_company_id);
            }
          }
        }
    });
  //输入信息获取剩余信息
  function obtain_info(id, x ,obj, bill_id,from_company_id){  //id 查询条件， table的x列数

    $.get("index.php?mod=warehouse&con=WarehouseBillInfoHA&act=getGoodsInfoByGoodsID", {goods_id: id, bill_id:bill_id,from_company_id:from_company_id }, function(data){
