$import(["public/js/select2/select2.min.js"],function(){
	var info_form_id = 'material_bill_info_rk';//form表单id
	...
	var obj = function(){
		var initElements = function(){
			...
			$("select[name='bill_type']").change(function(e){
				var select_ojb = $(this);
				if($(this).val()=='WH'){					
				    $.ajax({
				        type: 'post',
				        url: '/index.php?mod=warehouse&con=MaterialBillJin&act=getBillList&bill_type=WP',
				        data: {},  
				        success: function(result) {				        	
				            if (result.error == 0) {
				                var options_bill_type ="";				               		               
				                $.each(result.data,function(index,value){
								    options_bill_type = options_bill_type + "<option value='"+ value +"'>"+ value +"</option>";
								});
								$("select[name='from_bill_no']").append(options_bill_type);
								
							
								$("select[name='from_bill_no']").change(function(i){
									
									var bill_no = $(this).val();	
                                       									
										$.ajax({
											type: 'post',
											url: '/index.php?mod=warehouse&con=MaterialBillJin&act=getBillInfo',
											data: {bill_no:bill_no},  
											success: function(result1) {											    	
												if (result1.error == 0) {
													$("select[name='department_id']").select2('val',result1.department_id);
													$("select[name='warehouse_id']").select2('val',result1.warehouse_id);
												} else {
...
//			});	
		...
		var handleForm = function(){
			var url = info_form_base_url+(info_id ? 'update' : 'insertRK');
			var options1 = {
				url: url,
				error:function ()
				{
						...
						util.xalert(
							...
							function(){								
								...
								{//刷新首页
									...
									var tab_url ='index.php?mod=warehouse&con=MaterialBillJin&act=show&id='+data.bill_id;
