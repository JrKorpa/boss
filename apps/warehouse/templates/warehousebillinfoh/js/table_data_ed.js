// 表格初始化
var from_table_data_h = function (id, tdata, ttitle, ttype) {
  var $container = $(id);
  //表格基本内容初始化
  ...
  // 批量获取添加输入值
    function getBatchInfo(idArry,x,n,obj){  //id 查询条件， table的x行数
        ...
        var to_customer_id =$("#to_customer_id").val();
        var to_company_id = $("#to_company_id").val();
        $.get("index.php?mod=warehouse&con=WarehouseBillInfoH&act=getGoodsInfos", {'g_ids':idArry,'bill_id':bill_id,'to_customer_id':to_customer_id,'to_company_id':to_company_id}, function(data){
