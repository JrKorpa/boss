...
$import(["public/js/select2/select2.min.js",
	...
	var WarehouseBillCEditobj = function(){
		...
		$('#warehouse_bill_h_info_edit select').select2({
				placeholder: "请选择",
				allowClear: true
			}).change(function(e){
				$(this).valid();
		...
		var from_table = function(){
			...
			$.ajax({
				...
				success:function(res) {
					...
					//from_table_data_h_edit(res.id,res.data_bill_h,res.title,res.columns , info_id);
				}
			});
			//alert(121212);
			//保存值from_table_data_e
			$("body").find("#from_table_data_btn_h_edit").click(function() 
			{

					if ($("#warehouse_bill_h_info_edit #from_table_data_h_edit").find("td").hasClass("htInvalid") == true) 
					{
						$("#warehouse_bill_h_info_edit #from_table_data_h_edit").prev("p").addClass('text-danger').text("表单有错误信息，请更正再保存！");
						return false;
					}
					var save = {
						'data':$("#warehouse_bill_h_info_edit #from_table_data_h").handsontable('getData'),
						'id':info_id,
						'bill_note':$("#warehouse_bill_h_info_edit [name='bill_note']").val(),
						'create_time_start':$("#warehouse_bill_h_info_edit [name ='create_time_start']").val(),
