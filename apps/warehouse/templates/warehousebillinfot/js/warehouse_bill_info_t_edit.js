...
function downLoadEditExcel_t(obj){
	...
	var goods_sn = $('#warehouse_bill_t_edit input[name="bill_no"]').val();
	var url = $(obj).attr('data-url')+'&order_id='+goods_sn+'&order_type='+order_typt;
	location.href=url;
}

function importEditJs_t(o){
	...
	$.post($(o).attr('data-url'),{goods_sn:goods_sn},function(data){
		if(data==0){
			...
			$('#imjs_t').removeClass('red').addClass('green').attr('disabled','disabled');
...
$import(["public/js/select2/select2.min.js",
	...
	var obj = function(){
		var initElements = function(){
			$('#warehouse_bill_t_edit select').select2({
				placeholder: "请选择",
				allowClear: true
			}).change(function(e){
				$(this).valid();
		...
		var handleForm = function(){
			var url = 'index.php?mod=warehouse&con=WarehouseBillInfoT&act=update';
			var options1 = {
				url: url,
				error:function ()
				{
			...
			$('#warehouse_bill_t_edit').validate({
				...
				submitHandler: function (form) {
					$("#warehouse_bill_t_edit").ajaxSubmit(options1);
				}
			});
		};
		var initData = function(){
			util.batchCopyGoodsid('<%$view->get_id()%>','batch_copy_goodsid_t_edit');
			var url = 'index.php?mod=warehouse&con=WarehouseBillPay&act=show';
			$.post(url,{bill_id:info_id},function(data){
				if(data.success==1){
					$('#warehouse_bill_pay_e_t').html(data.content);
				}
				else{
					bootbox.alert(data.error ? data.error : ( data ? data : '程序异常'));
				}
		...
		var from_table = function(){
			$.ajax({
				url:"index.php?mod=warehouse&con=WarehouseBillInfoT&act=mkJson",
				dataType:"json",
				type:"POST",
				data:{'id':info_id},
