 ...
 *  -------------------------------------------------
 */
class WarehouseBillAController extends CommonController
{
	...
	public function search ($params)
	{
		...
		$page = isset($_REQUEST["page"]) ? intval($_REQUEST["page"]) : 1 ;
		//$model = new WarehouseBillModel(21);
		$model = new WarehouseBillAModel(55);//只读数据库
	...
	************************************************************************************************/
	...
	public function download($data,$goods_id) {
		...
		if($goods_id){
			if ($data['data']) {
				$down = $data['data'];
				$xls_content = "单据编号,单据类型,状态,货品数量,订单号,销售渠道,出库公司,入库公司,入库仓,供应商,原始成本价,	销售价,送货单号,制单时间,审核时间,备注,货号,款号,主成色重,证书号,手寸,长度,主成色,主石,主石重,主石粒数,主石净度,主石颜色,副石,副石粒数,副石重,总重,名称\r\n";
			...
			if ($data['data']) {
				$down = $data['data'];
				$xls_content = "单据编号,单据类型,状态,货品数量,订单号,销售渠道,出库公司,入库公司,入库仓,供应商,原始成本价,	销售价,送货单号,制单人,制单时间,审核人,审核时间,备注,货号,款号,主成色重,证书号,手寸,长度,主成色,主石,主石重,主石粒数,主石净度,主石颜色,副石,副石粒数,副石重,总重,名称\r\n";
				foreach ($down as $key => $val) {
