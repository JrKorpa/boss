 ...
 *  -------------------------------------------------
 */
class MaterialBillJinController extends CommonController
{
	...
	public function index ($params)
	{
		...
		$this->render('material_bill_search_form.html',array(
		    'bar'=>Auth::getBar(),
		    'view'=>new MaterialBillJinView($model)
		));
	}
	...
	public function searchBillGoods ($params)
	{
	    ...
	    $model = new MaterialBillJinModel($bill_id,21);
	    ...
	    $this->render('material_bill_goods_search_list.html',array(
	        'pa'=>Util::page($pageData),
	        'page_list'=>$data,
	        'billView'=>new MaterialBillJinView($model)
	...
	public function addBillGoods(){
	    ...
	    $bill_id = _Request::getInt('_id');
	    $billModel = new MaterialBillJinModel($bill_id,21);
	    $billView = new MaterialBillJinView($billModel);
	...
	public function insertBillGoodsRK($params){
	    ...
	    try{
	        $billGoodsModel = new MaterialBillGoodsJinModel(21);
	...
	protected function buildBillGoodsListRK($bill_info,$bill_type){
	    ...
	    $line = 0;
	    $billGoodsModel = new MaterialBillGoodsJinModel(21);
	    $billModel = new MaterialBillJinModel(21);
	...
	protected function buildBillGoodsListCK($bill_info,$bill_type){
	    ...
	    $line = 0;
	    $billGoodsModel = new MaterialBillGoodsJinModel(21);
	    $inventoryModel = new MaterialInventoryJinModel(21);
	    foreach ($goods_qty_list as $key=>$goods_qty){
	        $line++;
	        if(empty($goods_qty)){
	            continue;
	...
	public function addRK ()
	{
		...
		$result['content'] = $this->fetch('material_bill_info_rk.html',array(
			'view'=>new MaterialBillJinView(new MaterialBillJinModel(21))
		));
		$result['title'] = '添加入库单';
		Util::jsonExit($result);
	}
	...
	public function insertRK ($params)
	{
	    ...
	    $newdo = $res['data'];
	    $newmodel =  new MaterialBillJinModel(22);
	    try{
	        $pdolist[22] = $newmodel->db()->db();
	        Util::beginTransaction($pdolist);
	
	        $insert_id = $newmodel->saveData($newdo,$olddo);
	        $newmodel = new MaterialBillJinModel($insert_id,22);
	...
	public function edit ($params)
	{
		...
		$view = new MaterialBillJinView(new MaterialBillJinModel($id,21));
	...
	public function show ($params)
	{
		...
		$model=new MaterialBillJinModel($id,21);
		$view = new MaterialBillJinView($model);
	...
	public function update ($params)
	{
		...
		$newmodel =  new MaterialBillJinModel($id,22);
	...
	public function deleteBillGoods ($params)
	{
		...
		$model = new MaterialBillGoodsJinModel($id,22);		
		$res = $model->delete();
		if($res !== false){
			$result['success'] = 1;
		}else{
	...
	public function printBill(){
        $id = _Request::get('id');
        $billModel = new MaterialBillJinModel($id,21);
        $data = $billModel->getDataObject();
        $view = new MaterialBillJinView(new MaterialBillJinModel($id,21)); //获取单头信息
