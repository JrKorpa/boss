...
class AppPriceByStyleModel extends Model {
    ...
    public function updateWarehouseGoodsAgeDelete($id)
    {
        ...
        set 
            asg.is_delete = 2,wga.style_kuanprice_id=0,wga.is_kuanprice=0,wga.kuanprice=0
        where 
    ...
    public function updateWarehouseGoodsAgeRecover($info)
    {
        ...
        $sql = "
            ...
            where 
                ...
                AND wg.zuanshidaxiao >= $start
                AND wg.zuanshidaxiao <=  $end
                AND wg.zhushixingzhuang = '{$xz}'
                $zs_sql
                AND wg.zhushiyanse in ('$yanse')
                AND wg.zhushijingdu in ('$jingdu')
                AND wg.is_on_sale = 2
