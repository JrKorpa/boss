 ...
 *  -------------------------------------------------
 */
class WarehouseImportModel extends Model
{
     ...
     * @return boolean
     */
	function addGoodsData($goods_list,$oldPdo)
	{
		...
		try{
			...
			//老系统PDO数据库连接对象
			$oldPdo->setAttribute(PDO::ATTR_AUTOCOMMIT,0);//关闭sql语句自动提交
			$oldPdo->beginTransaction();//开启事务
			...
			foreach($goods_list as $key => $value)
			{
                ...
                $addtime = empty($value['addtime'])?$time:$value['addtime'];
				$sql = "select id,is_on_sale from `warehouse_goods` where goods_id = '".$value['goods_id']."'";
                $row=$this->db()->getRow($sql);        
				$warehouse_goods_id = null;//warehouse_goods表主键ID
				if($row!=false)//如果已经导入了，重复的，就跳过。
				{    
					if($row['is_on_sale']==100){
					    $warehouse_goods_id = $row['id'];
	                }else{    
					    continue;
					}    
				}
				...
				$sql_arr=array(
					...
					'is_on_sale'=>$value['is_on_sale'] == 1?2:100,
				...
				$data_warehouse_goods_id = $warehouse_goods_id?$warehouse_goods_id:'NULL';
				$sql = "REPLACE INTO `warehouse_goods` (id,"."`".implode('`,`',array_keys($sql_arr))."`".") VALUES (".$data_warehouse_goods_id.",'".implode("','",array_values($sql_arr))."'".");";
				//file_put_contents('c:/sql.txt',$sql."\r\n",FILE_APPEND);
				$pdo->query($sql);

                $sql = "select `id` from `warehouse_goods_age` where `goods_id` = '".$value['goods_id']."'";
                $age_id = $this->db()->getOne($sql);
				//将旧货号写入新系统库龄表，以便记录库龄；
                if(empty($age_id))
                {
                    $nowtime = time();
                    $total_age = ceil((($nowtime-strtotime($addtime))/86400));
                    $sql = "INSERT INTO `warehouse_goods_age` (`warehouse_id`, `goods_id`, `endtime`, `self_age`, `total_age`) VALUES ({$sql_arr['warehouse_id']} , '".$value['goods_id']."' , '0000-00-00 00:00:00' , '1' , '".$total_age."')";
                    $pdo->query($sql);
                }
				...
				$sql = "select id from goods_warehouse where good_id='{$sql_arr['goods_id']}'";
				$house_id = $this->db()->getOne($sql);
				
				$data_house_id = $house_id?$house_id:'NULL';
				$sql = "REPLACE INTO `goods_warehouse` (`id`,`good_id`, `warehouse_id`, `box_id`, `add_time`, `create_time`, `create_user`) VALUES ({$data_house_id},'{$sql_arr['goods_id']}',{$sql_arr['warehouse_id']},{$box_id},'{$time}','{$time}','SYSTEM')";
				//file_put_contents('c:/sql.txt',$sql."\r\n",FILE_APPEND);
				$pdo->query($sql);
				
				$sql = "update jxc_goods set is_on_sale = 100 where goods_id='{$sql_arr['goods_id']}'";
				$oldPdo->query($sql);
				
				$sql = "update warehouse_goods a,view_warehouse_goods b set a.caizhi = CONCAT(b.caizhi, b.jinse),a.goods_name=b.goods_name where a.goods_id=b.goods_id and a.goods_id='{$sql_arr['goods_id']}'";
				$pdo->query($sql);
			}
			...
			$oldPdo->commit();//如果没有异常，就提交事务
			$oldPdo->setAttribute(PDO::ATTR_AUTOCOMMIT,1);//开启sql语句自动提交
			return true;
		}
