 ...
 *  -------------------------------------------------
 */
class MaterialInventoryModel extends Model
{
	function __construct ($id=NULL,$strConn="")
	{
		$this->_objName = 'material_inventory';
		$this->pk='id';
		$this->_prefix='';
...
"cost_price"=>"成本单价");
	...
	{	    
		...
		if(empty($where['group_by'])){
			$sql = "SELECT inv.*,{$mg_fields} FROM `".$this->table()."` as inv inner join material_goods mg on inv.goods_sn=mg.goods_sn left join warehouse as wa on wa.id=inv.warehouse_id left join kela_supplier.app_processor_info as kr on kr.id = inv.supplier_id ";
	        $sqltj="select sum(inv.inventory_qty) as allnum,sum(round(inv.cost*inv.inventory_qty,2)) as allcost FROM `".$this->table()."` as inv inner join material_goods mg on inv.goods_sn=mg.goods_sn left join warehouse as wa on wa.id=inv.warehouse_id left join kela_supplier.app_processor_info as kr on kr.id = inv.supplier_id ";
		}else{
            $sql = "SELECT inv.goods_sn,inv.warehouse_id,inv.cost,sum(inv.inventory_qty) as inventory_qty,wa.name as warehouse_name,mg.style_sn,mg.style_name,mg.goods_name,mg.goods_spec,mg.catetory1,mg.catetory2,mg.catetory3,mg.unit,mg.cost as goods_cost  FROM `".$this->table()."` as inv inner join material_goods mg on inv.goods_sn=mg.goods_sn left join warehouse as wa on wa.id=inv.warehouse_id left join kela_supplier.app_processor_info as kr on kr.id = inv.supplier_id ";
	        $sqltj="select sum(inv.inventory_qty) as allnum,sum(round(inv.cost*inv.inventory_qty,2)) as allcost FROM `".$this->table()."` as inv inner join material_goods mg on inv.goods_sn=mg.goods_sn left join warehouse as wa on wa.id=inv.warehouse_id left join kela_supplier.app_processor_info as kr on kr.id = inv.supplier_id ";
