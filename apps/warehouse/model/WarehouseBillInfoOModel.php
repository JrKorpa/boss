...
/**
 *  -------------------------------------------------
 *   @file		: WarehouseBillInfoOModel.php
 *   @link		:  www.kela.cn
 ...
 *  -------------------------------------------------
 */
class WarehouseBillInfoOModel extends Model
{
	...
	*********************************/
	...
	public function add_info_o($data,$info)
	{
		...
		try{
			...
			//货品状态改变
			$sql = "UPDATE `warehouse_goods` SET `weixiu_status`= 2 , `weixiu_company_id` = {$info['to_company_id']} , `weixiu_company_name` = '{$info['to_company_name']}' , `weixiu_warehouse_id` = {$info['to_warehouse_id']}  , `weixiu_warehouse_name` = '{$info['to_warehouse_name']}' WHERE `goods_id` in (".$str_goods_id.") and ifnull(weixiu_status,0)<>'2'";
			$res_rows=$pdo->exec($sql);
			if(count($data)<>$res_rows){
				//	 echo $sql;
				$pdo->rollback();//事务回滚
