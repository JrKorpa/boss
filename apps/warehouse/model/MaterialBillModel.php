 ...
 *  -------------------------------------------------
 */
class MaterialBillModel extends Model
{
	function __construct ($id=NULL,$strConn="")
	{
		$this->_objName = 'material_bill';
		$this->pk='id';
		$this->_prefix='';
...
"batch_sn"=>"销售单号");
	...
	{
		//不要用*,修改为具体字段
		$sql = "SELECT DISTINCT b.*,s.name as main_supplier_name,c.channel_name,if(p2.company_name is not null,p2.company_name,p.company_name) as company_name,p.company_type,'/' as goods_sn,'/' as goods_name,w.name as main_warehouse_name,'/' as goods_allnum,'/' as goods_allcost,'/' as goods_allshijia from material_bill b 
	...
	{
		//不要用*,修改为具体字段
		$sql = "SELECT b.*,g.goods_sn,g.goods_name,g.goods_spec,g.style_sn,g.style_name,g.catetory1,g.catetory2,g.catetory3,g.goods_sale_price,g.unit,s.name as main_supplier_name,c.channel_name,p.company_name,p.company_type,w.name as main_warehouse_name,sum(bg.num) as goods_allnum,sum(round(bg.num*bg.cost,2)) as goods_allcost,sum(round(bg.num*bg.shijia,2)) as goods_allshijia from material_bill b 
			LEFT JOIN material_bill_goods bg on b.id=bg.bill_id 
			LEFT JOIN material_goods g on bg.goods_sn=g.goods_sn 
			LEFT JOIN kela_supplier.app_processor_info s on b.supplier_id=s.id 
			LEFT JOIN cuteframe.sales_channels c on b.department_id=c.id 
	...
	function checkBillPass($bill_id,$transMode = true){
	    ...
	    try{
    	    ...
    	    $sql = "select * from material_bill where id={$bill_id}";
    	    ...
    	    $sql = "select * from material_bill_goods where bill_id={$bill_id}";
    	    ...
    	    if($bill_cat==1){
    	        ...
    	        foreach ($billGoodsList as $goods){ 
        	        ...
        	        if($bill_type=='WH' && !empty($billInfo['from_bill_no'])){
		        	        $sql ="select *from material_inventory where goods_sn='{$goods['goods_sn']}' and supplier_id='{$goods['supplier_id']}' and warehouse_id='{$goods['in_warehouse_id']}' and batch_sn='{$goods['batch_sn']}'";
		        	        $exist_inventory = $this->db()->getRow($sql);
		        	        if(!empty($exist_inventory)){
		        	        	$sql = "select *from material_inventory where id='{$exist_inventory['id']}' for update";
                                $pdo->query($sql);
                                $sql = "update material_inventory set inventory_qty=inventory_qty+{$goods['num']} where id='{$exist_inventory['id']}'";
		        	            $pdo->query($sql);
		        	        }else{
			        	        $sql = $this->insertSqlNew($inventoryData,"material_inventory");
			        	        $pdo->query($sql);
                            }
	        	    }else{
		        	        $sql = $this->insertSqlNew($inventoryData,"material_inventory");
		        	        $pdo->query($sql);
		        	        //更新货品成本价
		        	        $sql = "update material_goods set cost={$goods['cost']} where goods_sn='{$goods['goods_sn']}'";
		        	        $pdo->query($sql);	        	    	
	        	    }    
    	        }
    	    }else if($bill_cat==2){
    	        ...
    	        foreach ($billGoodsList as $goods){
                    ...
                    $sql = "select * from material_inventory where id={$inventory_id}";
                    ...
                    $sql = "update material_inventory set inventory_qty=inventory_qty-{$inventory_qty} where id={$inventory_id} and version='{$inventoryInfo['version']}'";
    	            $num = $pdo->exec($sql);
                    if($num==0){
    	    ...
    	    $sql = "update material_bill set bill_status=2,check_user='{$user}',check_time='{$time}' where id={$bill_id}";
    	    $pdo->query($sql);
    	    if($transMode==true){
	...
	function getTotal($bill_id){
           $sql="select sum(num) as allnum,sum(round(num*cost,2)) as allcost,sum(round(num*shijia,2)) as allshijia from material_bill_goods where bill_id='$bill_id'";
    ...
    */
    public function getBill_detail($bill_no){
    	$sql = "select g.goods_name,g.unit,bg.* from material_bill_goods  bg left join material_goods g on bg.goods_sn=g.goods_sn,material_bill b where bg.bill_id=b.id and b.bill_no='{$bill_no}'";
    ...
    */
    public function getBill_Info($bill_no){
    	$sql = "select b.* from material_bill b where b.bill_no='{$bill_no}'";
