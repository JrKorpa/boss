...
/**
 *  -------------------------------------------------
 *   @file		: NoaccountReturnBillModel.php
 *   @link		:  www.kela.cn
 ...
 *  -------------------------------------------------
 */
class NoaccountReturnBillModel extends Model
{
	...
	*********************************/
	...
	public function add_info_o($data,$info)
	{
		...
		try{
            ...
            $id = $pdo->lastInsertId();
            //添加无账退货货品
            $sql = "INSERT INTO `warehouse_shipping`.`virtual_return_goods` (`id`, `business_type`, `order_sn`, `return_status`, `style_sn`, `caizhi`, `ingredient_color`, `gold_weight`, `torr_type`, `product_line`, `style_type`, `finger_circle`, `credential_num`, `main_stone_weight`, `main_stone_num`, `deputy_stone_weight`, `deputy_stone_num`, `resale_price`, `out_goods_id`, `place_company_id`, `place_company_name`, `place_warehouse_id`, `place_warehouse_name`, `guest_name`, `guest_contact`, `return_remark`, `without_apply_time`, `apply_user`, `exist_account_gid`,weixiu_fee) VALUES (NULL, '{$data['business_type']}', '{$data['order_sn']}', '{$data['return_status']}', '{$data['style_sn']}', '{$data['caizhi']}', '{$data['ingredient_color']}', '{$data['gold_weight']}', '{$data['torr_type']}', '{$data['product_line']}', '{$data['style_type']}', '{$data['finger_circle']}', '{$data['credential_num']}', '{$data['main_stone_weight']}', '{$data['main_stone_num']}', '{$data['deputy_stone_weight']}', '{$data['deputy_stone_num']}', '{$data['resale_price']}', '{$data['out_goods_id']}', '{$data['place_company_id']}', '{$data['place_company_name']}', '{$data['place_warehouse_id']}', '{$data['place_warehouse_name']}', '{$data['guest_name']}', '{$data['guest_contact']}', '{$data['return_remark']}', '{$data['without_apply_time']}', '{$data['apply_user']}', '{$data['exist_account_gid']}',{$data['weixiu_fee']});";

            $pdo->query($sql);
            
            $g_id = $pdo->lastInsertId();
            $no_account_gid = date("Y").str_pad($g_id,6,0,STR_PAD_LEFT);
            $no_account_gid = SYS_SCOPE=="zhanting"?"8".$no_account_gid:$no_account_gid;            

            $sql = "UPDATE `warehouse_shipping`.`virtual_return_goods` set no_account_gid = '{$no_account_gid}' where id={$g_id}";
            $pdo->query($sql);
			//添加维修退货单
