$import("public/js/select2/select2.min.js",function(){
	var info_form_id = 'dealer_customer_manage_follow';//form表单id
	var info_form_base_url = 'index.php?mod=dealer&con=DealerCustomerManage&act=';//基本提交路径
	...
	var obj = function(){
		...
		var handleForm = function(){
			var url = info_form_base_url+(info_id ? 'update_follow' : 'insert_follow'),compare_url = url + '&compare=1';
			var options1 = {
				url: compare_url,
				error:function ()
				{
							...
							$.post(url,{},function(res){
								...
								util.xalert(
									...
									function(){
										...
										{//查看编辑
											util.retrieveReload();//刷新查看页签
											util.syncTab(res.tab_id);//刷新数据主列表，无法定位到分页（有可能数据列表页签已经关闭，也有可能是其他对象穿透查看，所以分页函数不一定存在）
										}
										else
										{
											if (info_id)
