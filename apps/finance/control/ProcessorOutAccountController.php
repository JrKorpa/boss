<?php
     ...
     *  -------------------------------------------------
     */
    class ProcessorOutAccountController extends CommonController
    {
        ...
        public function index ($params)
        {
         ...
         //   $model = new ProcessorOutAccountModel(29);
         //   $companys = $model->getJiagongshangCompany();
            $this->render(  'processor_out_account_search_form.html',array(
                            'bar'=>Auth::getBar(),
        ...
        public function search ($params)
        {
            ...
            $args = array(
            	...
            	'bill_type_info'  => _Request::get('bill_type_info'),
...
// 			var_dump( $data);exit;
            if($data['error'] != 1) {
                ...
                $pageData['jsFuncs'] = 'processor_out_account_search_page';
                $this->render('processor_out_account_search_list.html',array(
       ...
       function downCsv()
        {
            $args = array(
                ...
                'pay_channel'	=> _Request::get('pay_channel'),//渠道
                'fin_status'=> _Request::get('fin_status'),
    			'bill_status'=>  _Request::get('bill_status'),//业务审核状态                'account_type' => _Request::get('account_type'),
            ...
            if (is_array($newdata)){
                ...
                {
                    $data = ApiModel::warehouse_api(array("bill_no"), array($v['bill_no']), "getJinZhong");
                    $jinzhong = $data['data']['jinzhong'];
                    $val = array($v['bill_no'],$v['pro_name'],$dictModel->getEnum('warehouse.put_in_type',$v['put_in_type']),$v['create_time'],$v['check_time'],$v['goods_total'],$v['amount'],"$jinzhong",'',$v['bill_note']);
                    $val = eval('return '.iconv('utf-8','gbk',var_export($val,true).';')) ;
                    $content[] = $val;
                }
            }
            $model = new GoodsModel(29);
            $model->detail_csv('加工商出库结算',$title,$content);
        }
    }
