$import("public/js/select2/select2.min.js",function(){
	...
	var obj = function(){
		var initElements = function(){
			$('#app_apply_bills_info select[name="supplier_id"],#app_apply_bills_info select[name="pay_type"]').select2({
				placeholder: "请选择",
				allowClear: true
			}).change(function(e){
				$(this).valid();
		...
		var handleForm = function(){
			...
			$('#'+info_form_id).validate({
				...
				rules: {
					...
					detail_data:{required:true}
				},
				messages: {
					supplier_id:{required:"请选择供应商"},
					pay_type:{required:"请选择应付类型"},
					detail_data:{required:"请选择上传数据"}
				},
