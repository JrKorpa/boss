$import(["public/js/select2/select2.min.js"],function(){
	var info_form_id = 'jxs_area_scope_info';//form表单id
	var info_form_base_url = 'index.php?mod=finance&con=JxsAreaScope&act=';//基本提交路径
	var info_id= '<%$view->get_id()%>';
    var pro_province= '<%$view->get_province_id()%>';
    var pro_city= '<%$view->get_city_id()%>';
    var pro_district= '<%$view->get_region_id()%>';
...
//			}
            ...
            $('#'+info_form_id+' select[name="mem_province_id"]').select2({
                placeholder: "请选择",
                allowClear: true,
                value: pro_province
            }).change(function(e){
                $(this).valid();
                $('#'+info_form_id+' select[name="mem_city_id"]').empty();
                $('#'+info_form_id+' select[name="mem_district_id"]').empty();
                $('#'+info_form_id+' select[name="mem_city_id"]').append('<option value=""></option>');
                var _t = $(this).val();
                if (_t) {
                    $.post('index.php?mod=finance&con=JxsAreaScope&act=getCity', {province: _t}, function(data) {
                        $('#'+info_form_id+' select[name="mem_city_id"]').append(data);
                        if (_t == pro_province) {
                            $('#'+info_form_id+' select[name="mem_city_id"]').select2("val", pro_city, true);
                        }
                        $('#'+info_form_id+' select[name="mem_city_id"]').change();
                    });
                }
                else {
                    $('#'+info_form_id+' select[name="mem_city_id"]').change();
            ...
            $('#'+info_form_id+' select[name="mem_city_id"]').select2({
                placeholder: "请选择",
                allowClear: true,
                value: pro_city
            }).change(function(e){
                $(this).valid();
                $('#'+info_form_id+' select[name="mem_district_id"]').empty();
                $('#'+info_form_id+' select[name="mem_district_id"]').append('<option value=""></option>');
                var _t = $(this).val();
                if (_t) {
                    $.post('index.php?mod=finance&con=JxsAreaScope&act=getDistrict', {city: _t}, function (data) {
                        $('#'+info_form_id+' select[name="mem_district_id"]').append(data);
                        if (_t == pro_city) {
                            $('#'+info_form_id+' select[name="mem_district_id"]').select2("val", pro_district, true);
                        }
                        $('#'+info_form_id+' select[name="mem_district_id"]').change();
                    });
                }
                else {
                    $('#'+info_form_id+' select[name="mem_district_id"]').change();
