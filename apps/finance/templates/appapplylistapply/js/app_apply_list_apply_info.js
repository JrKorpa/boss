$import(["public/js/select2/select2.min.js"],function(){
	...
	var obj = function(){
		var initElements = function(){
			...
			$('#app_apply_list_apply_info select[name="prc_id"]').select2({
				placeholder: "请选择",
				allowClear: true
			}).change(function(e){
				$(this).valid();
			});
			$('#app_apply_list_apply_info select[name="type"]').select2({
				placeholder: "请选择",
				allowClear: true
			}).change(function(e){
				$(this).valid();
		...
		var handleForm = function(){
			var url = info_id ? 'index.php?mod=finance&con=AppApplyListApply&act=update' : 'index.php?mod=finance&con=AppApplyListApply&act=insert';
			var options1 = {
				url: url,
				error:function ()
				{
			...
			$('#app_apply_list_apply_info').validate({
				...
				submitHandler: function (form) {
					$("#app_apply_list_apply_info").ajaxSubmit(options1);
				}
			});
			//回车提交
			$('#app_apply_list_apply_info input').keypress(function (e) {
				if (e.which == 13) {
					$('#app_apply_list_apply_info').validate().form()
				}
			});
		};
...
function app_apply_list_apply_goods_download_mo_info()
{
