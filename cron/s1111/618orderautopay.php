...
$ids = $payModel->getalloutids();
//$ids = array('1236541348254579');
//$ids = array('26377862144994124','30586749879136173','9893198995812253');
if(empty($ids))
{
	exit('请输入您要点款的淘宝订单');
}
...
foreach($ids as $taobaoinfo)
{
	...
	//获取淘宝订单中真正需要付款的值
	//上次因为金额出错导致点款有问题所以点款根据淘宝流水账号去拿
	$tradmoney = $apiModel->get_order_info_payment($taobaoid);
	...
	if($real_payment < $kl_orderinfo["order_amount"] ){
		$real_payment = $kl_orderinfo["order_amount"];
	}
	
	/*618中接口返回的数据可能会小于订单的金额 ，因为订单金额增加了分摊的  淘宝需要承担的钱单独给淘宝去结算  所以都是全款
	if($real_payment != $kl_orderinfo['money_unpaid'])
	{
