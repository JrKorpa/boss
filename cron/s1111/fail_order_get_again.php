...
//每次都去跑上一次跑完后失败的订单
//如果我们是6小时跑一次  开始肯定是全部都跑一次
//$time = 6;
$nowtime = date('Y-m-d');
$failwhere['btime'] = date('Y-m-d',strtotime("$nowtime-6 hour"));
$failwhere['etime'] = date('Y-m-d');
//$failwhere = array();
$faildata = $orderModel->getfalsedata($failwhere);
$issingle = 0;
if($faildata)
{
	while($failobj=$faildata->fetch_assoc())
	{
		$outsn = $failobj['out_order_sn'];
		array_push($allids,$outsn);
	}
}

if(empty($allids))
{
	exit('休息下吧,过去6小时没有抓取失败的订单了');
}
