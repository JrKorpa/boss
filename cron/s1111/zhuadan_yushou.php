...
if(!empty($requestid))
{
	...
	$s_data = date('Y-m-d H:i:s',strtotime("$nowtime-1 day"));
	$e_data = date('Y-m-d H:i:s');
	//$s_data = '2018-10-15 00:00:00';
	$e_data = '2018-11-10 23:23:59';
...
foreach($allids as $taobaoid)
{
	...
	if($open_trade_status)
	{
		...
		if($open_ys_status)
		{
			...
			{
				$data['out_order_sn'] = $taobaoid;
				$data['reason'] = '淘宝订单:'.$taobaoid.'状态出错 我不抓取';
	...
	foreach($goodsarr as $goodsinfo)
	{
		...
		if($taobaoid=='52197573093647734')
			$flag="预售".$flag."hy";

		if(strpos($flag,'预售')!==false)
			$yshou = true;
		//if(mb_substr(trim($flag), 0, 2, 'utf8')=='预售')
		//	$yshou = true;
	...
	//如果开启了预售订单 
	if($yshou===true && trim(@$orderinfo->trade->status) != "WAIT_BUYER_PAY")
	{
		//外部导单成功与否记录日志
    ...
    if($yshou!==true){
    	//echo "暂不抓取非预售单".PHP_EOL;
        //continue;  
    }  	

	//如果开启了预售订单 
	if($yshou===false && trim(@$orderinfo->trade->status) != "WAIT_SELLER_SEND_GOODS")
	{
		//外部导单成功与否记录日志
		$record['out_order_sn']= $taobaoid;
    ...
    //订单满即送  根据付款时间送赠品
	$ordersprice_gifts = getgiftbyprice($id,$createname,$orderinfo->trade->payment,$orderinfo->trade->pay_time,$yshou);
    ...
    if($yshou===true){
			$yushou_gift = array();
			$yushou_gift['goods_sn'] = 'KLWL000086';
			$yushou_gift['name'] = '凉生钥匙扣';
			$yushou_gift['desc'] = '预售单默认送赠品KLWL000086';
			$ys_gifts = combinedata($id,$createname,$yushou_gift);
			if(!empty($ys_gifts))
			{				
				$giftinfo = $ys_gifts['order_gift'];
				
				$res_gift = $orderModel->autoinsert('app_order_details',$giftinfo);
				if($res_gift)
				{
					echo '预售单默认赠品录入app_order_details成功<br/>'.PHP_EOL;
				}else{
					$stepflag = false;
					$reason .= '预售单默认赠品录入app_order_details失败<br/>';
					echo '预售单默认赠品录入app_order_details失败<br/>'.PHP_EOL;
				}							
			}
	}
	//预售赠品结束					
