...
if(!empty($requestid))
{
	array_push($allids,$requestid);
	$issingle = 1;
}else{
	$nowtime = date('Y-m-d H:i:s');
	//$s_data = date('Y-m-d H:i:s',strtotime("$nowtime-2 day"));
    $s_data = '2018-11-11 00:00:00';
	$e_data = date('Y-m-d H:i:s');

	//$e_data = '2018-11-11 01:00:00';
    ...
    while($has_next){
    	...
    	$res = $apiModel->getTaobaoOrderList_1111($s_data,$e_data,$i);
    	//print_r($res);
    	$i = $i+1;    
    	...
    	if(property_exists($res,'trades')){
    		$trade = $res->trades->trade;
    		if(is_array($trade)){
	            foreach($trade as $order){
	            	array_push($allids,$order->tid);
	            } 
	        }else{
	        	array_push($allids,$trade->tid);
	        }    
    	}
    	if($i==120)
    		$has_next = false;
...
//$allids =array();
//$allids = ['236901037973012623'];

if(empty($allids))
{
	exit('has not orders...休息下吧,没有需要发货的淘宝订单了');
}
...
foreach($allids as $taobaoid)
{
	...
	{
		echo '订单'.$taobaoid.'已经抓取过了,不需要重复抓取,<br/>'.PHP_EOL;
		continue;
	}
	echo '订单id:'.$taobaoid.'开始抓取<br/>'.PHP_EOL;
	//开始通过淘宝订单id  获取淘宝订单信息
    ...
    //if($taobaoid=='87795830234117766')
    //   print_r($orderinfo);
    if(!empty($orderinfo->trade->num_iid)){
	    if(in_array(trim($orderinfo->trade->num_iid),['41324571790','19678010799','3873545362','577822990893','18615450236','15584144765','14560987817','526072018375'])){
			$error = '商品ID在41324571790,19678010799,3873545362,577822990893,18615450236,15584144765,14560987817,526072018375特殊处理';
			$data['out_order_sn'] = $taobaoid;
			$data['order_status'] = $orderinfo->trade->status;
			$data['reason'] = $taobaoid.'商品ID在41324571790,19678010799,3873545362,577822990893,18615450236,15584144765,14560987817,526072018375特殊处理<br>';
			$orderModel->recordInsert($data);
			if($issingle>0)
			{
				echo $data['reason'];
	...
	if(in_array($buyer_message,$company_message))
	{
		...
		if($issingle>0)
		{
			echo $data['reason'].PHP_EOL;
		}
		continue;
	}

	if(strpos($buyer_message,'体验店')!==false){
		$data['out_order_sn'] = $taobaoid;
		$data['reason'] = '淘宝订单:'.$taobaoid.'买家备注:xxx体验店,我不抓取';
		$data['order_status'] = $orderinfo->trade->status;
		$orderModel->recordInsert($data);
		if($issingle>0)
		{
	...
	//拿取客服备注信息 卖家备注 $orderinfo->trade->seller_memo
	/*
	if(empty($orderinfo->trade->seller_memo))
	{
		$data['out_order_sn'] = $taobaoid;
		$data['reason'] = '淘宝订单:'.$taobaoid.'卖家备注为空,我不抓取';
		$data['ispreorder'] = 0;
		$data['order_status'] = $orderinfo->trade->status;
		$orderModel->recordInsert($data);
		if($issingle>0)
    ...
    $seller_memo_except_exit = false;
	foreach ($seller_memo_except as $key => $item) {
		if(strpos($remark,$item)!==false){
			$data['out_order_sn'] = $taobaoid;
			$data['reason'] = '淘宝订单:'.$taobaoid.'卖家备注:['.$remark.']含['.$item.'],我不抓取';
			$data['order_status'] = $orderinfo->trade->status;
			$orderModel->recordInsert($data);
			if($issingle>0)
			{
			...
			$seller_memo_except_exit = true;
			continue;		
		}
	}
    if($seller_memo_except_exit==true)
    	continue;
	
	//为了和顾客写贺卡里面用中文逗号,所以不做中文逗号转换
	//如果没有填写备注的情况下,自动默认(针对静默订单)  那就没有提成了哟
	if($remark=="")
	{
		$remark="亢思迪,无,无,KLBZ";
	}
	...
	//区
	$region_regional = !empty($orderinfo->trade->receiver_district) ? trim($orderinfo->trade->receiver_district) : '';
	$regiondata = $orderModel->getcityids($region_province,$region_city,$region_regional);
	
	//淘宝订单商品信息
	//$goodsarr = $orderinfo->trade->orders->order;
	$goodsarr = array();
	if(is_array($orderinfo->trade->orders->order))
		$goodsarr = $orderinfo->trade->orders->order;
	else
		$goodsarr[0] = $orderinfo->trade->orders->order;
    ...
    if(count($remark_list)<>count($goodsarr)){
		$data['out_order_sn'] = $taobaoid;
		$data['reason'] = '淘宝订单:'.$taobaoid.'商品数量和订单备注的商品数量不一致,我不抓取';
		$data['ispreorder'] = 0;
		$data['order_status'] = $orderinfo->trade->status;
		$orderModel->recordInsert($data);
		if($issingle>0)
	...
	foreach($goodsarr as $goodsinfo)
	{
		...
		if(in_array(trim($goodsinfo->num_iid),['41324571790','19678010799','3873545362','577822990893','18615450236','15584144765','14560987817','526072018375'])){
			$errorinfo = $taobaoid.'商品ID在41324571790,19678010799,3873545362,577822990893,18615450236,15584144765,14560987817,526072018375特殊处理<br>';
			break;	
		}
		//print_r($goodsinfo);
		//商品标题
		...
		//echo "[".$goodsinfo->sku_properties_name]";
		$smarkname = $goodsinfo->sku_properties_name;
		if($remark == "亢思迪,无,无,KLBZ")
		{
			$remark_list[$i] = '亢思迪,无,无';
		}
		$remark = $remark_list[$i];
		$i++;
		
		//商品的款号
		//$tmpgoodssn = (property_exists($goodsinfo,'outer_sku_id') && !empty($goodsinfo->outer_sku_id)) =="" ? $goodsinfo->outer_sku_id : $goodsinfo->outer_iid;
		$tmpgoodssn = '';
		if(!empty($goodsinfo->outer_sku_id)){
			$tmpgoodssn = $goodsinfo->outer_sku_id;
		}else{			
			$errorinfo = $flag."货品没有配置款号";
            //continue;
            $tmpgoodssn = '';
		}
		//如果是银饰吊坠款  那么就不用去过滤了 直接根据款号拿取
	    ...
	    if(!empty($remark)){
		    $strPattern = "/(?:发票抬头\[)(.*)(?:\])/i";
		    $pattern_arr = array();    
	        preg_match_all($strPattern, $remark, $pattern_arr);
	        if(!empty($pattern_arr[1][0]))
	        	$invoice_title = $pattern_arr[1][0];
	    }    	
	...
	if(!empty($errorinfo))
	{
		...
		$record['reason'] = $order_type.$taobaoid.$errorinfo;
		$record['order_status'] = $orderinfo->trade->status;
		$orderModel->recordInsert($record);
		if($issingle>0)
		{
	...
	$baseinfo['check_user'] = date("Y-m-d H:i:s");
	
	//修改时间
	$baseinfo['modify_time'] = date("Y-m-d H:i:s");
	//备注信息
	$temp_remark = !empty($orderinfo->trade->seller_memo) ? $orderinfo->trade->seller_memo :'';
	$baseinfo['order_remark'] = '买家备注为:'.$buyer_message.'<br/>卖家备注为:'.$temp_remark;
	//录单来源
	...
	echo '第一步录入订单基本信息表<br/>:'.PHP_EOL;
	$res1 = $orderModel->autoinsert('base_order_info',$baseinfo);
	$id = $res1;  //自动生成的订单id
	if($res1)
	{
		echo '录入base_order_info成功<br/>'.PHP_EOL;
	}else{
		$stepflag = false;
		$reason .= '录入base_order_info失败<br/>'.PHP_EOL;
		echo '录入base_order_info失败<br/>'.PHP_EOL;
	}
	...
	foreach($goodsarr as $k=>$goodsinfo)
	{
		...
		//加上使用了优惠券的金额
		$goodsallyh = bcadd($goodsallyh, (!empty($goodsinfo->part_mjz_discount) ? $goodsinfo->part_mjz_discount : 0),2);
		
		
		//商品sn
		$goods_sn = $goodsinfo->outer_sku_id;
		...
		//需要取多少条
		$result = $orderModel->getgoodsinfo($newwhere,$g_num,$taobaoid);
		$g_infos = array();
		...
		//商品的优惠等于商品优惠+优惠券的钱
		$allyouhui = bcadd($goodsinfo->discount_fee,(!empty($goodsinfo->part_mjz_discount) ? $goodsinfo->part_mjz_discount : 0),2);
		...
		for($i=0;$i<$g_num;$i++)
		{
			...
			$details['zhiquan'] = '';
			$details['xiangkou'] = '';
			$details['chengbenjia'] = '0';	
			$details['is_stock_goods'] = 0;
			$details['goods_type'] = 'style_goods';
			$details['tuo_type'] = '成品';
            unset($details['zhushi_num']);
            if(!empty($newwhere['zuanshidaxiao'])){
            	$details['zhushi_num'] = !empty($newwhere['zhushi_num']) ? $newwhere['zhushi_num'] : 1;

            }
				//商品的总额
			$goodstotalprice = bcadd($goodstotalprice,$goodsinfo->price,2);
			//是否是现货：1现货 0期货
		    $details['is_stock_goods'] = 0;          //默认为1 如果查不到信息就为期货0
			if(!empty($g_infos) && isset($g_infos[$i]))
			{
				...
				$details['clarity'] = $g_data['zhushijingdu'];
				$details['color'] = $g_data['zhushiyanse'];
				$details['cert'] = $g_data['cert'];
				$details['zhengshuhao'] = $g_data['zhengshuhao'];
				if(!empty($newwhere['jinliao']))
                    $details['caizhi'] = $newwhere['jinliao'];
				if(!empty($newwhere['jinse']))
                    $details['jinse'] = $newwhere['jinse'];                
				...
				if(isset($newwhere['jinse']) && !empty($newwhere['jinse']))
				{
					$details['jinse'] = $newwhere['jinse'];
				}
				if(isset($newwhere['zuanshidaxiao']) && !empty($newwhere['zuanshidaxiao']))
				{
					$details['cart'] = $newwhere['zuanshidaxiao'];
					//如果出现净度  在这里是期货 那就把镶嵌要求改了
					$details['xiangqian'] = '工厂配钻，工厂镶嵌';					
				}
				if(isset($newwhere['zhushijingdu']) && !empty($newwhere['zhushijingdu']))
				{
					$details['clarity'] = $newwhere['zhushijingdu'];
				}
				if(isset($newwhere['zhushiyanse']) && !empty($newwhere['zhushiyanse']))
				{
					$details['color'] = $newwhere['zhushiyanse'];					
				}
				if(isset($newwhere['gold_weight_min']) && !empty($newwhere['gold_weight_min']))
				{
					$details['jinzhong'] = $newwhere['gold_weight_min'];
				}
				
				$details['zhiquan'] = isset($newwhere['shoucun']) ? $newwhere['shoucun']:'';
			}
			$details['kezi'] = isset($newwhere['ziyin']) ? $newwhere['ziyin']:'';
			...
			if($res2)
			{
				echo '录入app_order_details成功<br/>'.PHP_EOL;
				if($first_detail_id==0)
					$first_detail_id = $res2;
				if(isset($details['goods_id']) && $details['goods_id']>0)
				{
					//修改商品绑定订单
					$orderModel->updateorderid($res2,$details['goods_id']);
					echo '货号'.$details['goods_id'].'已经绑定订单商品自增id'.$res2.'<br/>'.PHP_EOL;
				}
			}else{
				$stepflag = false;
				$reason .= "录入app_order_details失败<br/>".PHP_EOL;
				echo '录入app_order_details失败<br/>'.PHP_EOL;
			}

			//这里是每个商品的备注赠品信息入库   因为不能直接插入赠品数量 所以是多条记录
			/**********          商品备注赠品开始          **********/
				...
				if(!empty($zparr))
				{
					...
					foreach($zparr as $zpv)
					{
						...
						if(!empty($bz_gifts))
						{
							...
							if($res_gift)
							{
								echo '商品备注赠品录入app_order_details成功<br/>'.PHP_EOL;
							}else{
								$stepflag = false;
								$reason .= '商品备注赠品录入app_order_details失败<br/>';
								echo '商品备注赠品录入app_order_details失败<br/>'.PHP_EOL;
							}
							...
							if($res_relgift)
							{
								echo '商品备注赠品录入rel_gift_order成功<br/>'.PHP_EOL;
							}else{
								$stepflag = false;
								$reason .= '商品备注赠品录入rel_gift_order失败<br/>';
								echo '商品备注赠品录入rel_gift_order失败<br/>'.PHP_EOL;
							}*/
						}
						/**********          商品备注赠品结束          **********/
					}
			...
			$goods_gifts = getgiftinfo($id,$goodstitle,$createname,$goods_sn);
			if(!empty($goods_gifts))
			{
				...
				if($res_gift)
				{
					echo '商品赠品录入app_order_details成功<br/>'.PHP_EOL;
				}else{
					$stepflag = false;
					$reason .= '商品赠品录入app_order_details失败<br/>';
					echo '商品赠品录入app_order_details失败<br/>'.PHP_EOL;
				}
				/*
				//////////          赠品入库rel_gift_order表          
				...
				if($res_relgift)
				{
					echo '商品赠品录入rel_gift_order成功<br/>'.PHP_EOL;
				}else{
					$stepflag = false;
					$reason .= '商品赠品录入rel_gift_order失败<br/>';
					echo '商品赠品录入rel_gift_order失败<br/>'.PHP_EOL;
				}*/
			}
	...
	if($orderisxh < 1 )
	{
		...
		echo '修改订单:'.$order_sn.',变成期货单<br/>'.PHP_EOL;
	}
	...
	if($res3)
	{
		echo '录入app_order_account成功<br/>'.PHP_EOL;
	}else{
		$stepflag = false;
		$reason .= "录入app_order_account失败<br/>";
		echo '录入app_order_account失败<br/>'.PHP_EOL;
	}
	...
	//区
	$regional = !empty($orderinfo->trade->receiver_district) ? trim($orderinfo->trade->receiver_district) : '';
	...
	$phone = !empty($orderinfo->trade->receiver_phone) ? trim($orderinfo->trade->receiver_phone) : '';
	...
	if($res6)
	{
		echo '录入app_order_address成功<br/>'.PHP_EOL;
	}else{
		$stepflag = false;
		$reason .='录入app_order_address失败<br/>';
		echo '录入app_order_address失败<br/>'.PHP_EOL;
	}
	...
	//发票抬头
	$invoice['invoice_title'] = $invoice_title; //默认填写个人
	//发票内容
	...
	//计算淘宝平台承担的优惠费用
	$promotion_taobao =0;
	if(!empty($orderinfo->trade->promotion_details->promotion_detail)){		
        $promotion_detail = $orderinfo->trade->promotion_details->promotion_detail;
        foreach ($promotion_detail as $k_d => $promotion) {
       	   if(in_array($promotion->promotion_name,$taobao_promotion)){
       	   	    $promotion_taobao = $promotion_taobao + $promotion->discount_fee;
       	   }
        }
	...
	//发票金额减去淘宝平台承担的优惠费用
    $invoice['invoice_amount'] = bcsub($invoice['invoice_amount'],$promotion_taobao,2);
	...
	if($res4)
	{
		echo '录入app_order_invoice成功<br/>'.PHP_EOL;
	}else{
		$stepflag = false;
		$reason .='录入app_order_invoice失败<br/>';
		echo '录入app_order_invoice失败<br/>'.PHP_EOL;
	}
	...
	if($res5)
	{
		echo '录入rel_out_order成功<br/>'.PHP_EOL;
	}else{
		$stepflag = false;
		$reason .='录入rel_out_order失败<br/>';
		echo '录入rel_out_order失败<br/>'.PHP_EOL;
	}
    ...
    //订单满即送  根据付款时间送赠品
	$ordersprice_gifts = getgiftbyprice($id,$createname,$orderinfo->trade->payment,$orderinfo->trade->pay_time);
		...
		foreach ($ordersprice_gifts as $z => $ordergiftdata) {
		    ...
		    if(!empty($ordergiftdata['details_remark'])){
		        ...
		        if($res){
					...
					$stepflag = false;
					$reason .= '订单无款号赠品更新app_order_details备注失败<br/>';
	...
	if($res6)
	{
		echo '录入s11_order_info成功<br/>'.PHP_EOL;
	}else{
		$stepflag = false;
		$reason .='录入s11_order_info失败<br/>';
		echo '录入s11_order_info失败<br/>'.PHP_EOL;
	}
	...
	if($res8)
	{
		...
		$stepflag = false;
		$reason .= "app_order_action操作日志添加失败<br/>".PHP_EOL;
	...
	if($stepflag)
	{
		echo '淘宝订单'.$taobaoid.'抓单ok对应的BDD订单是'.$order_sn.'<hr/>'.PHP_EOL;
	}
}
?>