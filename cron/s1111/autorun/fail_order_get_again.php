...
$nowtime = date('Y-m-d');
$failwhere['btime'] = date('Y-m-d',strtotime("$nowtime-10 day"));
...
if($faildata)
{
	while($failobj=$faildata->fetch_assoc())
	{
		...
		array_push($ids,$outsn);
	}
}
if(empty($ids))
{
	exit('没有需要回写的订单了都已经成功了');
}
	...
	foreach($goodsarr as $k=>$goodsinfo)
	{
		...
		for($i=0;$i<$g_num;$i++)
		{
			...
			if(!empty($g_infos) && isset($g_infos[$i]))
			{
				...
				if(isset($where['gold_weight_min']) && !empty($where['gold_weight_min']))
				{
					$details['jinzhong'] = $where['gold_weight_min'];
				}
