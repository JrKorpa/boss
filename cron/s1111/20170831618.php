...
foreach($allids as $obj)
{
	...
	$goods_sn  = empty($goods_sn) ? 'KLGX000002' : $goods_sn;
	//如果商品数量出现多个就进行价格拆分
	$g_num = $goodsinfo->num;
	...
	//创建人
	$details['create_user'] = '辛娟';             //备注里面的客服
	//
...
function step_order($order_sn,$buyer_message,$orderinfo)
{
	...
	//制单人
	$baseinfo['create_user'] = '辛娟';  //默认辛娟
	//增加审核时间和审核人
	$baseinfo['check_time']= date("Y-m-d H:i:s");
	$baseinfo['check_user'] = 'admin';
	//修改时间
...
function calcprice($promotion_detail){
	$tb_ob = array('天猫购物券黄金珠宝专用','天猫购物券服饰大额专用');
	$pt_ob = array('618购物券');
