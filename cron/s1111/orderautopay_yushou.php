...
foreach($ids as $taobaoinfo)
{
    ...
    //付款余额
    $deposit = 	bcsub($real_payment , $kl_orderinfo['money_paid'],2);
	if(bccomp($deposit,$kl_orderinfo['money_unpaid'],2)!=0)
	{
		echo 'BDD后台未付款金额不等于淘宝实际付款金额,'.$taobaoid.'点款失败<br/>'.PHP_EOL;
		continue;
	}
	if(bccomp($deposit,$kl_orderinfo['money_unpaid'],2)==1){
	...
	$pay_action = array(
		'Payaction'=>array(
			...
			"deposit" => $deposit,
			"order_amount" => $real_payment,
			"balance" => bcsub($real_payment,$deposit,2),
		...
		if($flag)
		{
			...
			$payModel->updateOutOrder($deposit,$kl_orderinfo['order_id']);
			$pay_stu = $payModel->changgestu_yushou($kl_orderinfo['order_id']);
			//操作日志
			...
			$ation['remark'] = "外部订单[".$taobaoid."]，通过外部订单支付了".$deposit." 元";
			//4
