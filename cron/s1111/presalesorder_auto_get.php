...
auto: liulinyan
date: 2015-10-17
file: presalesorder_auto_get.php
used: 淘宝预售单,单独抓单脚本
*/
...
include_once(API_ROOT.'PreOrderClassModel.php');
//引入规则文件
...
//设置各个开关
$open_check = false;           //是否开启淘宝订单抓取过滤
//获取所有的订单id
//$s_data = date('Y-m-d').' 00:00:00';
$s_data = '2015-10-29 00:00:00';
$e_data = '2015-10-29 23:00:00';
//$e_data = date('Y-m-d').' 23:59:59';
//对于预售单来说 只需要抓取等待买家付款的即可
$orderstatus = 'WAIT_BUYER_PAY';
$ids = $apiModel->getTaobaoOrderList($s_data,$e_data,1,100,$orderstatus);
//内部推广的用户备注
...
//新品银质吊坠 压缩版 KLPW029315 立体版 KLPW029314
...
$orderModel = new PreOrderClassModel();
$i=0;
foreach($ids as $taobaoid)
{
	...
	if(trim($orderinfo -> code))
	{
		...
		$data['reason'] = '没有找到淘宝订单:'.$taobaoid.'或者接口获取数据异常';
		$orderModel->recordInsert($data);
		continue;
	}
	//为了避免状态错误  再次获取
	if(trim(@$orderinfo->trade->status) != "WAIT_BUYER_PAY")
	{
		$data['out_order_sn'] = $taobaoid;
		$data['reason'] = '预售订单:'.$taobaoid.'不是等待买家付款状态 我不抓取';
		$data['order_status'] = $orderinfo->trade->status;
		$orderModel->recordInsert($data);
		continue;
	}
	//买家备注
	$buyer_message = empty($orderinfo->trade->buyer_message)?'':trim($orderinfo->trade->buyer_message);
	...
	//如果卖家备注出现"电子凭证"四个字就不去匹配货品,并且不能点款
	$ordertype ='';
	if(strpos($remark,'电子凭证')!=false)
	{
		$ordertype ='核销单';
	...
	if(!$yshou)
	{
		...
		$record['reason'] = '淘宝订单:'.$taobaoid.'是等待买家付款状态,但不是预售单';
	...
	//录单来源
	$baseinfo['referer'] = empty($ordertype)?'双11抓单':$ordertype;
	//订单状态0有效1删除
	$baseinfo['is_delete']=0;             //默认为0
	//申请关闭:0=未申请，1=申请关闭 
	$baseinfo['apply_close'] = 0;
	//是否是现货：1现货 0定制
	$baseinfo['is_xianhuo'] =  1;         //默认为1
	//是否打印提货单 0否 1是
	...
	foreach($goodsarr as $k=>$goodsinfo)
	{
		...
		//是否是现货：1现货 0期货
		$details['is_stock_goods'] = $isxianhuo;          //默认为0
		//退货产品 0未退货1已退货
		...
		for($i=0;$i<$g_num;$i++)
		{
			...
			if(!empty($g_infos) && isset($g_infos[$i]))
			{
				...
				if(isset($newwhere['jinliao']) && !empty($newwhere['jinliao']))
				{
					$details['caizhi'] = $newwhere['jinliao'];
				}
				if(isset($newwhere['yanse']) && !empty($newwhere['yanse']))
				{
					$details['jinse'] = $newwhere['yanse'];
				}
				if(isset($newwhere['zuanshidaxiao']) && !empty($newwhere['zuanshidaxiao']))
				{
					$details['cart'] = $newwhere['zuanshidaxiao'];
				}
				if(isset($newwhere['clarity']) && !empty($newwhere['clarity']))
				{
					$details['clarity'] = $newwhere['clarity'];
					//如果出现净度  在这里是期货 那就把镶嵌要求改了
					$details['xiangqian'] = '工厂配钻，工厂镶嵌';
				}
				if(isset($newwhere['color']) && !empty($newwhere['color']))
				{
					$details['color'] = $newwhere['color'];
					//如果出现净度  在这里是期货 那就把镶嵌要求改了
					$details['xiangqian'] = '工厂配钻，工厂镶嵌';
				}
			...
			if(isset($newwhere['caizhi']) && strpos($newwhere['caizhi'],'K'))
			{
				$giftcaizhi = '18K';
			}else{
				$giftcaizhi = '';	
	...
	//商品总额
	$account['goods_amount'] = $goodstotalprice;
	//订单优惠券金额
	$account['coupon_price'] = $yhqtotalprice;           //默认为0,不抓取
	//订单商品优惠金额
