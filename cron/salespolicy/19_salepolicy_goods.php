...
$sql = "SELECT count(*) FROM `base_salepolicy_goods` WHERE `isXianhuo`=1 and (`company` in ($company_in) or `warehouse` in ($warehouse_in)) and `product_type`=15";
...
for($ii = 1; $ii <= $forsize; $ii ++){
	$offset = ($ii - 1) * 1000;
    $sql = "SELECT * FROM `base_salepolicy_goods` WHERE `isXianhuo`=1 and (`company` in ($company_in) or `warehouse` in ($warehouse_in)) and `product_type`=15 limit $offset,1000";
