...
while ($obj = fgetcsv($file))
{
	...
	$tmp_product =  trim(iconv('gb2312','utf-8',$obj[4]));
	if($tmp_product !='' && in_array($tmp_product,array_keys($products)))
	{
		$where['product_type1'] = $products[$tmp_product];
	}
	$where['caizhi'] = trim(iconv('gb2312','utf-8',$obj[6]));
	$where['start'] =  $obj[8];
	$where['end'] = $obj[7];
	$price = trim($obj[9]);
	$Model->updateqhpricebysn($where,$price);
}
?>