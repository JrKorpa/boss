<?php
class ImportOrderClassModel extends PdoModel
{
	...
	public function recordInsert($data)
	{
		...
		$recordlog['order_status'] = isset($data['order_status']) ? $data['order_status']:'WAIT_SELLER_SEND_GOODS';
	...
	where a.order_goods_id < 1 and a.is_on_sale=2 and a.ziyin = '' and ";
		...
		{
			$sgoodsid = implode(',',$where['goods_id']);
			$sql .=" a.goods_id not in($sgoodsid) and ";
		}
