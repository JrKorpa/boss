...
/* $update_sql = "update `app_salepolicy_goods` set `is_delete`=2 WHERE `isXianhuo`=1";
...
//$where_in = iconv('utf-8','gbk',"'展会活动库','彩宝库','代销库','深圳珍珠库','线上钻饰库','线上唯品会货品库','线上混合库','淘宝素金','京东素金','线上低值库','淘宝黄金','京东黄金'");

$sql = "SELECT count(*) FROM `base_salepolicy_goods` WHERE `isXianhuo`=1 and `warehouse` in ($where_in) and `product_type`=6";
//echo $sql;die;
...
for($ii = 1; $ii <= $forsize; $ii ++){
	$offset = ($ii - 1) * 1000;
    $sql = "SELECT * FROM `base_salepolicy_goods` WHERE `isXianhuo`=1 and `warehouse` in ($where_in) and `product_type`=6 limit $offset,1000";
