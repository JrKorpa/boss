...
$sql = "SELECT count(*) FROM `base_salepolicy_goods` WHERE `isXianhuo`=1 and `warehouse` in ($where_in) and `product_type` in (16,17)";
//echo $sql;die;
...
for($ii = 1; $ii <= $forsize; $ii ++){
	$offset = ($ii - 1) * 1000;
    $sql = "SELECT * FROM `base_salepolicy_goods` WHERE `isXianhuo`=1 and `company_id`=58 and `warehouse` in ($where_in) and `product_type` in (16,17) limit $offset,1000";
