...
$sql = "SELECT count(*) FROM `base_salepolicy_goods` WHERE `isXianhuo`=1 and `product_type`=8 and `warehouse` in ($where_in)";
//echo $sql;die;
...
for($ii = 1; $ii <= $forsize; $ii ++){
	$offset = ($ii - 1) * 1000;
    $sql = "SELECT * FROM `base_salepolicy_goods` WHERE `isXianhuo`=1 and `product_type`=8 and `warehouse` in ($where_in) limit $offset,1000";
