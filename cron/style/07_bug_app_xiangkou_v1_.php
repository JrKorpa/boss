...
for($ii = 1; $ii <= $forsize; $ii ++){
    echo $ii."----------------------------------------------------------"."\n";
    $offset = ($ii - 1) * $num;
    $sql = "SELECT * FROM " . $old_table . " WHERE `is_new`=0  and style_sn='KLRW028163' and `style_cat`=1 limit $offset,$num";
    $sql = "SELECT * FROM " . $old_table . " WHERE `is_new`=0  and style_sn='KLRM027602' and `style_cat`=1 limit $offset,$num";
    $sql = "SELECT * FROM " . $old_table . " WHERE `is_new`=0   and `style_cat`=1  limit $offset,$num";
...
//        var_dump($is_zhushi,$zhushi_attr);
        ...
        foreach ($select_finger as $f_val){
            foreach ($select_stone as $s_val){
                ...
                $sql = "select x_id from `".$new_table."` where `stone`=".$s_val." and finger = '".$f_val."'and  `style_id`= ".$style_id;
