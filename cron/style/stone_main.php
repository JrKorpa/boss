...
function getNew(){
	...
	$sql = "select ss.style_id,si.style_sn,stone_cat as main_stone_cat,ss.stone_attr as main_stone_attr from `front`.`rel_style_stone` as ss,`front`.`base_style_info` as si where ss.style_id=si.style_id and stone_position=1 and stone_cat<>0 and is_new=0;";
	$arr = mysqli_query($db_new, $sql);
	$data= array();
...
function getOld(){
	$db_old = mysqli_connect('192.168.1.55', 'kela_styles', 'eZLrHTmDxDYZeWxF', 'kela_style');
