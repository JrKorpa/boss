...
$db_conf = [
	'boss' => [
		'dsn'=>"mysql:host=192.168.1.59;dbname=kela_supplier",
		'user'=>"cuteman",
...
$boss_db = new MysqlDB($db_conf[$scope]);
...
foreach($rows as $r) {
	notify_bcd_upserted($r['id'], $r['p_id'], array('sys_scope' => $scope), $boss_db);
}
...
function notify_bcd_upserted($bc_id, $order_detail_id, $data, $db) {
	...
	*/
		...
		if (empty($style_potential_diy_list))  {		
			...
			$db->exec("insert into fix_diy(dt, bc_id, s_val, c_val) select '".date('YmdH')."' ,{$bc_id}, 0, 1 from product_info where id = {$bc_id} and is_quick_diy = 1");
	...
	if ($is_quick_diy == 1) {
		...
		if ($dia_type == 1) {
			$db->exec("insert into fix_diy(dt, bc_id, s_val, c_val) select '".date('YmdH')."'  ,{$bc_id}, 1, 0 from product_info where id = {$bc_id} and is_quick_diy = 0");
		} else {
			$is_quick_diy = 0;
			$db->exec("insert into fix_diy(dt, bc_id, s_val, c_val) select '".date('YmdH')."' ,{$bc_id}, 0, 1 from product_info where id = {$bc_id} and is_quick_diy = 1");
		}
