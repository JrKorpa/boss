...
class MysqlDB { 
    ...
    public function update2($tabel,$set,$where)
    {
        if(empty($row) || !is_array($row) || empty($where) || !is_array($row)){
            return false;
        }
    ...
    public function insert2($table,$row)
    {
        if(empty($row) || !is_array($row)){
            return false;
        }
        return $this->autoExec2($row,$table);
    }
     ...
     * @return bool
     */
    public function autoExec2($data,$table,$act='INSERT',$where=array()){
        $fields = $this->getFields($table);
        $param = array();//print_r($data);exit;
        foreach ($fields as $v) {
            if(array_key_exists($v,$data)){
