...
class OrderGoodsVO {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['originMerItemNo'])){
				
				$this->originMerItemNo = $vals['originMerItemNo'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("originMerItemNo" == $schemeField){
				
				$needSkip = false;
				$input->readI64($this->originMerItemNo); 
	...
	public function write($output){
		...
		if($this->originMerItemNo !== null) {
			
			$xfer += $output->writeFieldBegin('originMerItemNo');
			$xfer += $output->writeI64($this->originMerItemNo);
