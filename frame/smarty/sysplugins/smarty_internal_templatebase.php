    ...
    public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false)
    {
        ...
        if (!$_template->source->exists) {
            if ($_template->parent instanceof Smarty_Internal_Template) {
				$dir = substr(str_replace('\\','/',$this->smarty->template_dir[0]),strlen(KELA_ROOT));
                $parent_resource = " in '".$dir."{$_template->parent->template_resource}'";
				throw new SmartyException("Unable to load template {$_template->source->type} '".$dir."{$_template->source->name}'{$parent_resource}");
           } else {
                $parent_resource = '';
