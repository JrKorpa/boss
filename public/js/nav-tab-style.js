...
var tab_delete = function() {
	$('#nva-tab').on('click', 'li > .close', function (e) {
		e.preventDefault();
                ...
                var yangfuyou = 1;
                while(true){
                        if(util.getItem('data'+yangfuyou)){
                                util.deleteItem('data'+yangfuyou);
                                yangfuyou++;
                        }
                        else
                        {
...
var tab_nav = function() {
	$('.sub-menu').on('click', 'ul > li > a', function (e) {
		...
		var is_out = $(this).attr('is_out');

                if(parseInt(is_out))
                {
                        window.open(url,$title,'fullscreen:true,menubar:false,resizable:false,titlebar:false,toolbar:false');
                }
                else
                {
                        //判断标签页是否存在
	...
	$('#tab-content').on('click', 'a.tab-con-a', function (e) {
		...
		$('#nva-tab li').each(function(){
			...
			var href = $(this).children('a').attr('href');
                        var ohref = href.substr(1);
                        
                        href = ohref.split('-');
			href.pop();
			href = href.join('_');

			if (href==prefix || $id==ohref)
			{
				flag=true;
				var that = this;
				bootbox.confirm({  
