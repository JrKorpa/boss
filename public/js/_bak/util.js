...
util._load = function(obj,type){
	...
	util._sync(url,$(obj).parent().parent().siblings().find('table'),type);
}
...
util.moveup = function(obj){
	...
	$.post(url,{id:id},function(data){
		...
		if (data.success==1)
		{
			util._sync(util.getItem("url"),$(obj).parent().parent().parent().parent().parent(),false);
		}
		else
		{
			util.error(data);
