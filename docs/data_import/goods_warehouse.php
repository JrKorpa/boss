...
// $sql = "SELECT `g`.`id`,`g`.`goods_id`,`g`.`warehouse_id`,`b`.`id` AS `box_id` FROM `warehouse_goods` AS `g`,`warehouse_box` AS `b` WHERE `g`.`box_sn`=`b`.`box_sn` and `g`.`warehouse_id`=`b`.`warehouse_id` AND `g`.`old_set_w`='0'";
...
$sql_test = $sql_1."".$sql_2;
 //print_r($sql_test);exit;
	...
	if($res){
		...
		file_put_contents(__DIR__."/log/insert/g_".date('Ymd_')."_sql.log",$sql_test."\r\n",FILE_APPEND);
	}
...
file_put_contents(__DIR__."/log/insert/g_".date('Ymd_')."_sql.log","\r\n---===THE MISSION END===---\r\n||||||||||||||||||||||||||||||||||||||||||||||\r\n||||||||||||||||||||||||||||||||||||||||||||||\r\n||||||||||||||||||||||||||||||||||||||||||||||\r\n\r\n\r\n",FILE_APPEND);
