...
class ExportData
{
    ...
    public function getOldDate($where = false,$pass = false,$box = false)
    {
        ...
        if($pass !== false){
            $sel .= ",`".$this->old_pk."`";
            $p_str = " AND `g`.`".$this->pass_status."` = '0'";
        }

        if($box){
            $sql = "SELECT ".$sel." FROM ".$this->o_table." AS `g` WHERE `tmp_sn` <> '' GROUP BY `tmp_sn`";
        }else{
            $sql = "SELECT ".$sel.",g.`addtime`,c.`p_name` AS company,w.`wh_name` AS warehouse FROM ".$this->o_table.' AS `g` LEFT JOIN `jxc_processors` AS `c` ON (g.`company` = c.`p_id`) LEFT JOIN `jxc_warehouse` AS `w` ON (`g`.`warehouse` = `w`.`wh_id`) WHERE 1=1';
    ...
    public function insertData($dict = array(),$save_create = false,$default=array())
    {
        if(empty($this->old_data)){
            echo iconv('UTF-8', 'GBK', 'NO DATA TO INSERT');
            exit;
        }
            ...
            foreach ($this->old_data as $key=> $row) {
                foreach ($row as $k => $v) {
                    ...
                    if($k == 'shoucun'){
                        switch ($v) {
                            case '1106005932':
                                $this->old_data[$key][$k] = '0';
                                break;
                            case '1408254395':
                                $this->old_data[$key][$k] = '0';
                                break;
                            case '17#':
                                $this->old_data[$key][$k] = '17';
                                break;
                            case '18#':
                                $this->old_data[$key][$k] = '18';
                                break;          
                            default:
                                if($v == '' || $v = ' '){$this->old_data[$key][$k] = '0';  }
                        }
                    }
                    if(($k == 'jinhao')){
                        if(($v == '') || ($v == ' ')){$this->old_data[$key][$k] = "0";}
                    ...
                    if($k == 'box_sn'){
                        $this->old_data[$key][$k] = $row[$k]?$row[$k]:"0-00-0-0";
                    }
                }
        ...
        //=================预处理=======================
            ...
            foreach ($this->old_data[0] as $k => $v) {
                //不记录`oldsys_id`
                // if(!$this->old_id && ($k == $this->oldsys_id)){
                //     continue;
                // }else{
                    $label .= "`".$k."`,";
                    $value .= ":".$k.","; 
        ...
        //=================绑定参数=====================
            foreach ($this->old_data[0] as $key => $value) {
                // if(!$this->old_id && ($key == $this->oldsys_id)){
                //        continue;
                // }else{             
...
//        }
    ...
    /*写默认柜*/
    public function insertWarehouseBox(){
        ...
        if(!empty($data)){
            $i = 0;
            foreach ($data as $row) {
                $sql = "SELECT count(*) FROM `warehouse_box` WHERE `warehouse_id` = '".$row['id']."' AND `box_sn` = '0-00-0-0'";
                $res = $this->new_db->query($sql)->fetch()[0];
                if(!$res){
                    $sql = "INSERT INTO `warehouse_box` (`warehouse_id`,`box_sn`,`create_time`,`create_name`,`is_deleted`,`info`) VALUES ('".$row['id']."','0-00-0-0',"."'".date('Y-m-d H:i:s')."'".",'system','1','默认柜')";
                    // print_r($sql);exit;
                    $res = $this->new_db->query($sql);
                    if($res){$i++;
                        echo iconv('UTF-8', 'GBK', "SUCCESS CREATE ".$i." DEFAULT BOX \r\n");
                    }else{
                        echo iconv('UTF-8', 'GBK', "---=== SUCCESS CREATE DEFAULT BOX ===---\r\n\r\n\r\n\r\n\r\n");
                    }
                }
