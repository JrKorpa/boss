...
class ExportData
{
    ...
    public function getOldData()
    {
        ...
        $sql = "SELECT ";
        $sql .= $sel;
        $sql .= " ,p.p_name AS company,j.wh_name AS warehouse FROM `jxc_goods`AS m,`jxc_processors` AS `p`,`jxc_warehouse` AS j,`tmp1_diff` AS d";
    ...
    /*旧项目临时表*/
    public function create2Tmp($base_path = false)
    {   
        ...
        //临时表 tmp1
            ...
            if($res){
                $sql = "CREATE TABLE tmp1 as SELECT g.goods_id as old_gid FROM jxc_order AS o, jxc_order_goods AS og,jxc_goods as g WHERE o.order_id = og.order_id AND og.goods_id = g.goods_id AND o.status <> '3' AND o.type <>'W' AND o.addtime >= '".$time."' GROUP BY g.goods_id";
        ...
        print_r($sql);exit;
            ...
            $this->csv_file = $file = $base_path.$base_name."tmp_2diff.csv";
            $sql = 'SELECT * FROM `tmp_2diff` into outfile "'.$file.'"';
            $res3 = $this->old_db->query($sql);
    ...
    {   
        ...
        foreach ($data as $row) {
            ...
            if($res){
                echo iconv('UTF-8', 'GBK', "SUCCESS UPDATE ".$where['goods_id']." TO [".$this->o_table."]\r\n");
            }else{
