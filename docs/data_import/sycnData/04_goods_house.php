...
foreach ($data as $row) {
	...
	if($res){
		$s++;
		echo $s."SUCCESS UPDATE `goods_warehouse` SET warehouse_id = ".$row['w_house']." WHERE `goods_id` = ".$row['goods_id']."<br/>\r\n";
	}else{
		$l++;
		echo "LOSE";
		file_put_contents(__DIR__."/log/update/G_house_".date('Ymd_H').".log",$row['goods_id']."<br/>\r\n",FILE_APPEND);
	}
}
echo iconv('UTF-8', 'GBK', "---===THE MISSION END===---<br/>\r\n");
echo iconv('UTF-8', 'GBK', "SUCCESS UPDATE ".$s." LINE <br/>\r\n");
echo iconv('UTF-8', 'GBK', "LOSE UPDATE ".$l." LINE <br/>\r\n");
